...
> 对比对象：VCPChat、VCPToolBox、Hermes Agent、Open WebUI、LobeHub、AstrBot、OpenClaw
>
> 对比更新日期：2026-09-04
>
> 依据：同目录七份基于各项目当前源码快照的单项目调查笔记
>
...
七个样本均已确认至少一条主动运行主链，但它们的工作单元并不相同。Hermes、Open WebUI、LobeHub、VCPToolBox 与 OpenClaw 都有独立日程或任务对象；VCPChat、Hermes、AstrBot 与 OpenClaw 还能把触发重新接入既有会话；VCPToolBox、Open WebUI 与 AstrBot 另有邮件、日历或群聊等条件唤醒，OpenClaw 另有 hook/webhook 外部事件唤醒。比较时，触发脱离用户回合是共同条件，不代表各样本都有同样的耐久性、取消能力或结果可见性。

隔离日程运行最适合比较持久状态与恢复。Hermes 的 cron 以 profile 文件、执行账本和认领协议隔离运行；Open WebUI 把每次 Automation 记录为运行历史并生成真实 chat 或 channel；LobeHub 将任务、task topic 和 Brief 放入 PostgreSQL；VCPToolBox 则以 JSON 任务规则和 history 汇总 Agent 派发结果；OpenClaw 以 SQLite 同时持有 cron job、run receipt、task run 与 detached subagent run，isolated/current 运行使用独立 run session，main 目标则经 heartbeat 回到主会话。AstrBot 的 cron 定义和最近状态持久化，但没有单独的运行账本，运行仍重新进入原会话。

会话内续作强调连续性而非后台队列。VCPChat FlowLock 的运行 Session 只在渲染进程内存在，能恢复尚未认领的跨 Topic 请求，不能恢复已运行 Session。Hermes heartbeat 把下一次触发状态保存到会话数据库，但需要 CLI 或 Gateway 轮询器继续运行。OpenClaw 的 cron main 与 heartbeat 也走会话续作：main job 只把 system event 放入目标会话再请求 heartbeat，heartbeat monitor 是系统拥有的持久 cron job，触发定义与执行账本（receipt、task run、heartbeat outcome）都持久化。AstrBot cron 也以合成事件延续原消息会话，和前两者不同的是其触发定义由持久 cron job 管理。

取消语义普遍分为“阻止下一轮”与“中断当前执行”两层。七个样本都能从静态代码确认前者的至少一部分；Hermes cron 与 OpenClaw 的 cron/hook 运行取消、后台 exec 的进程终止都已追踪到取消事件进入 Agent/脚本执行或真实进程路径。其余样本对已开始的模型流、Agent 或外部工具是否被完整中断，均保留为未验证或未追踪边界。
...
隔离日程运行
  ...
  `- OpenClaw: cron job -> receipt/task run -> isolated/detached run session 或 main heartbeat 回合
...
会话内续作
  ...
  `- OpenClaw: cron main/heartbeat -> system event -> main session
...
条件唤醒或进程内后台工作
  ...
  `- OpenClaw: hook/webhook -> 一次性 cron run 或 wake -> heartbeat/isolated run
```

同一项目可落入多个分型。例如 Hermes 同时具有会话 heartbeat 与隔离 cron；VCPToolBox 的邮件投递不是 TaskAssistant 的任务记录；AstrBot 的后台工具有运行 ID，却没有可恢复的持久任务对象。OpenClaw 的分型差异由 sessionTarget 决定：同一份 cron 定义既可回主会话（main）也可走独立 run session（isolated/current），heartbeat monitor 本身又是系统拥有的持久 cron job。
...
- [OpenClaw 单项目笔记](OpenClaw-主动Agent与后台任务调查笔记.md)：cron、heartbeat、hooks、后台 exec、detached subagent 与 durable delivery。
