...
导出源是单个会话。顺序与范围由 `createMessagesList(history, history.currentId)` 决定（`src/lib/utils/index.ts:1404-1418`）：沿 `parentId` 从当前消息回溯到根再反转，即只包含活动分支路径，不包含隐藏分支；JSON 导出例外，它导出整个 `ChatResponse`（含完整 `history` 树），只是下载层面不做任何裁剪。

权限门：分享按钮要求 admin 或 `user.permissions.chat.share`（前端 `Menu.svelte:369`），后端再校验 `chat.share`（`backend/open_webui/routers/chats.py:1940`）；导出对应 `chat.export`（`ChatMenu.svelte:319`）。env 默认 `USER_PERMISSIONS_CHAT_SHARE/EXPORT/IMPORT=True`、`ALLOW_PUBLIC_SHARING/ALLOW_OPEN_SHARING=False`（`backend/open_webui/config.py:1854-1866`）。

## 2. 范围选择、内容口径与字段过滤

- TXT（两个菜单组件实现一致，`ChatMenu.svelte:64-87`）：逐条拼接 `### 角色` + `getOutputText(message.output) || message.content`。`getOutputText` 只取 `output` 中 `type === 'message'` 项的文本（`Messages/structuredOutput.ts:335-341`）；无 output 的消息退回原始 `content`——注意这里没有 `removeAllDetails`（聊天渲染端 `ResponseMessage.svelte:189` 是 `getOutputText(...) || removeAllDetails(message.content)`），因此若 content 内含 `<details type="reasoning">` 等标记，会原样进入 TXT 与纯文本 PDF。此为静态代码事实，实际消息中 reasoning 的落库形态未运行确认。
...
- 快照分享：`SharedChats.create` 复制 `chat.chat`（整个会话 JSON，含 history、messages、params、system 等）到 `shared_chat.chat` 列（`backend/open_webui/models/shared_chats.py:60-89`），不做任何内容过滤。
- 全量导出 `GET /chats/all` 以 NDJSON 流式输出（`chats.py:988-993`）；`GET /chats/all/db` 为 admin + `ENABLE_ADMIN_EXPORT` 门控（`chats.py:1029-1033`）。
...
- 样式化 PDF：捕获的是完整 `Messages` 渲染结果，消息内嵌的 `<img>` 会出现在 canvas 中；`html2canvas-pro` 配置了 `useCORS: true`，但跨域图片若无 CORS 头仍可能污染 canvas（基于库行为推断，未运行验证）。
...
- JSON：单会话数组与全量导出一致，可经 DataControls 导入（`POST /chats/import`，`chats.py:788-809`）或侧边栏拖拽导入完成往返；导入端兼容 OpenAI 导出格式（`getImportOrigin`/`convertOpenAIChats`，`DataControls.svelte:42-48`）。导出结果采用服务端 Chat 模型可接受的原始结构，字段多于精简 messages payload，往返语义在单仓库内闭合。整库迁移、配置导出（`/configs/export`）不在本类目。
- TXT：面向阅读的 Markdown 风格文本，无 schema，无导入路径。
- PDF：A4 竖版 JPEG 光栅页（样式模式）或纯文本页（文本模式），无书签/元数据/可复制文本（样式模式）。
- 统计导出（社区相关）：`GET /chats/stats/export` 返回 JSON 分页或 NDJSON 流（`ChatStatsExport`，含 `chat: ChatBody` 全量会话），受 `ui.enable_community_sharing` 门控（`chats.py:581-619`）。
...
- 样式模式（默认）：`showFullMessages=true` 渲染 `#full-messages-container`（内嵌聊天同款 `Messages` 组件，readOnly）→ `cloneNode` 克隆到 body（绝对定位、`left:-9999px`、固定宽 800px）→ 把 `.message-listitem` 的 `contentVisibility` 强制为 visible 以对抗聊天列表的渲染优化 → `html2canvas-pro(clone, {backgroundColor 按暗色主题, useCORS, scale:2, width:800})` → jsPDF A4；按 `canvas.width/210mm` 折算每页像素高，把整张 canvas 切成等高条，每条 `toDataURL('image/jpeg', 0.7)` 后 `addImage`，暗色模式先铺黑底矩形（`Menu.svelte:138-197`）。
- 文本模式（`stylizedPdfExport=false`）：取 TXT 文本，jsPDF 8pt `splitTextToSize` 自动换行分页。
- 富内容保真：样式模式复用聊天现场渲染器，reasoning（`<details type="reasoning">`）、工具卡、代码高亮、数学、表格等与屏幕所见一致（无需专门降级）；文本模式全部退化为纯文本。后端 `utils/pdf_generator.py` 是第三套实现（fpdf2 + markdown 库，`content.replace('\n','<br/>')` 极简排版），仅被 `/utils/pdf` 暴露，前端 `downloadChatAsPDF` 包装存在但全仓库无调用点（grep `downloadChatAsPDF(` 无结果，`src/lib/apis/utils/index.ts:98-123`）。
- 无图片/PNG 导出、无分享图生成（对比 NextChat 的 E3 图片链，Open WebUI 本次未找到）。

## 7. 生成历史、版本与持久化

不适用。重新生成即重新下载，无结果列表；分享侧"重新生成"是覆盖语义：`POST /chats/{id}/share` 在 `chat.share_id` 已存在时走 `SharedChats.update` 重拍快照并保留同一 URL（`chats.py:1947-1959`），UI 上表现为"Update and Copy Link"按钮（`ShareChatModal.svelte:220-224`）。旧快照不保留（除非源会话自身分支）。无分享版本历史、无过期时间设置。

## 8. 分享载体、访问控制与撤销

- 载体：站内快照 URL `{origin}/s/{uuid4}`（`ShareChatModal.svelte:31`），token 为 UUID，无枚举目录；`/s/[id]` 页面（`src/routes/s/[id]/+page.svelte`）无 `+page.server.ts`，纯客户端拉取 `GET /chats/share/{share_id}`。
- 快照语义：`GET /chats/share/{share_id}` 通过 `Chats.get_chat_by_share_id` 返回 `shared_chat` 快照行的 ChatModel 视图（`backend/open_webui/models/chats.py:1519-1538`）；它与源会话分离，源会话后续变化不自动反映，需显式“更新链接”重新快照。分享时复制会话 JSON，不复制附件。
- 可见性三态（`AccessControl.svelte:217-245` + `filter_allowed_access_grants`，`backend/open_webui/utils/access_control/__init__.py:220-289`）：
  - private：无 `*` 授权，仅显式 user/group 授权可读；
  - public：`user:* read`，任一登录用户可读（`AccessGrants.has_access` 的 public 分支，`backend/open_webui/models/access_grants.py:562-620`）；
  - open：`anyone:* read`，匿名可读（`is_open_shared_chat` → `has_anyone_access`，`chats.py:81-87`、`access_grants.py:540-560`）。
  服务端会按用户权限过滤掉无权授予的级别（`filter_allowed_access_grants`）；owner 与 admin（`ENABLE_ADMIN_CHAT_ACCESS` 时）总可读（`chats.py:90-103`）。
- 撤销与管理：单聊删除 `DELETE /chats/{id}/share`（清快照行、清 `share_id`、清授权，`chats.py:1983-2005`）；全部撤销 `DELETE /chats/share/all`（`chats.py:1123-1148`）；列表管理入口在 Settings → Shared Chats（`SharedChatsModal.svelte`，分页+搜索+逐条/全部取消）。无过期机制。
- 克隆：`POST /chats/{id}/clone/shared` 把快照（或 admin 下任意 chat）以 `originalChatId` + 分支指针导入为访问者自己的会话（`chats.py:1822-1889`），分享页提供"Clone Chat"按钮（`s/[id]/+page.svelte:148-164`）。
...
- `/s` 页面 `<meta name="robots" content="noindex,nofollow">`（`s/[id]/+page.svelte:173`），无站点级 /share 目录或搜索索引（全仓未找到 share 列表公开页；`GET /chats/shared` 仅返回当前用户自己的分享，`chats.py:1151-1180`）。
...
- 样式 PDF 为单次全量 canvas（scale 2、宽 800px），长会话的高度与内存取决于 canvas 上限，代码无分段再拼或压缩中间态（除分页切条外）；异常路径 `catch` 只 `console.error`，且克隆节点清理与 `showFullMessages` 复位只写在成功路径（`Menu.svelte:186-191`）——若 `html2canvas` 抛错，离屏克隆节点可能残留 body。此为静态推断，未运行验证。
