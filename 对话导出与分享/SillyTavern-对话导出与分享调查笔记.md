...
- JSONL 导出是持久化原始数据的逐字节交付：服务端对聊天文件直接 `fs.readFileSync` 回传（`src/endpoints/chats.js:625-643`），包含 header 行（`chat_metadata`）、系统消息、完整 `swipes`/`swipe_info`、`extra` 附件引用与分支字段。点击导出前客户端先 `saveChatConditional()` 强制落盘（`public/script.js:11443`），保证导出即磁盘最新状态。
...
- 角色/群聊信息以消息级字段携带：每条消息 `name`（说话人）随 JSONL 原样保留，群聊消息另有 `original_avatar`/`force_avatar` 头像引用（`public/scripts/group-chats.js:594-608`）；群成员配置在独立 group 文件中，不进入聊天导出；角色卡/提示词快照不嵌入（角色资产交换属 Agent 角色类目）。
- swipe/branch：JSONL 原样导出保留同一消息的全部 swipe 与 `swipe_info`；分支以独立聊天文件 + `chat_metadata.main_chat` 父引用 + 分支点 `extra.branches` 名单表达（`public/scripts/bookmarks.js:186-243`），导出按文件粒度进行，一次只导出一个文件、不打包分支树。swipe 选择器中的“导出”按钮被改造成“创建分支”操作（`public/scripts/swipe-picker.js:159-174`），不作为导出入口。
- 本次未找到：HTML/PDF/PNG 对话交付与长图生成（应用代码无 html2canvas/jsPDF/window.print 引用；`toDataURL` 仅用于视频缩略图与头像裁剪）、`navigator.share` 系统分享、Gist/公开链接/远端快照及对应访问控制。
...
- 主入口：“管理聊天文件”弹窗（`#option_select_chat` → `displayPastChats` → `displayChats`），每个聊天块渲染导出按钮（`public/index.html:6779-6780`、`public/script.js:8521-8558`）；按钮在键盘可聚焦序列中（`public/scripts/keyboard.js:25-26`）。本次未找到斜杠命令、消息级菜单或 Chat UI 中的其他导出入口。
- 导出源：磁盘上的单个完整聊天文件（`.jsonl`），不接受内存选区或可见消息子集；角色聊天与群聊共用同一模板与点击处理器，群聊请求带 `is_group: true`、`avatar_url` 为空（`script.js:11447-11453`），服务端按 `is_group` 选择 chats 或 groupChats 目录（`chats.js:608-614`）。
...
- TXT 口径 = 可见正文：跳过 `is_system` 消息（`chats.js:653-655`），输出 `${name}: ${(data.extra.display_text || data.mes)}`（`chats.js:656-660`）；`extra.display_text` 只由 translate 扩展写入（`public/scripts/extensions/translate/index.js:215`），因此译文存在时导出译文、否则导出原文。无标题、无日期、无模型/角色元信息。
- swipe 口径：持久化中 `mes` 与活动 swipe 互为镜像（`script.js:6837 syncMesToSwipe`、`6895 syncSwipeToMes`），故 TXT 只含活动 swipe；JSONL 含 `swipes` 全历史。
- Assistant 导出口径：内存 `chat` 减去 ASSISTANT_NOTE 类型系统消息（`chats.js:2147`），其余消息（含其他系统消息与 swipe）原样序列化。

## 3. 附件、资源与离线封装

- JSONL：附件以引用形式保留（消息 schema：`extra.media[].url`、`extra.files`、已弃用 `extra.image`/`extra.video`，`public/global.d.ts:107-119`），二进制实体不打包；交付物在其他安装或机器上无法解析附件，不能离线打开。
- TXT：附件无任何表达。
- 无内联、无打包、无 base64 嵌入路径。

## 4. 格式、schema 与往返能力

- 格式与命名：`jsonl`（原文件直读）、`txt`（投影正文）；导出文件名 `<聊天名>.jsonl` / `<聊天名>.txt`（`script.js:11451`），MIME 分别为 `application/octet-stream` 与 `text/plain`（`script.js:11469`）。
- schema：JSONL 首行为 `ChatHeader`（`chat_metadata` + 占位 `user_name`/`character_name`，`script.js:7368-7373`），其后每行一条 `ChatMessage`（`global.d.ts:66-81`）。无显式格式版本号，兼容性靠字段存在性判定。
- 往返（JSONL）：导入校验首行含 `user_name`/`name`/`chat_metadata` 任一（`chats.js:764`），非 Chub 数据时 `fs.copyFileSync` 逐字节落盘、未知字段完整保留（`chats.js:786`）；Chub 聊天数据会被拍平（`mes`/`swipes` 中的 `{message}` 对象包装转字符串，`chats.js:258-279`），是唯一改写路径。导入另存为新文件，不覆盖源（`chats.js:780-782`）。
- 往返（外部 JSON）：按结构特征识别并转换 Kobold Lite（`savedsettings`）、CAI Tools（`histories`）、oobabooga（`data_visible`）、Agnai（`messages`）、RisuAI（`type==='risuChat'`）（`chats.js:718-738`、转换器 `110-308`），全部投影为 `name/is_user/mes/send_date/extra` 简化消息，不保 swipe 与分支；CAI 支持多历史批量导入。
- 往返（群聊）：客户端对群聊禁用 json 格式（`script.js:12032-12035`）；服务端将文件直接复制进 groupChats 目录，客户端把新 chatId 登记进 `group.chats` 并保存群（`chats.js:676-694`、`group-chats.js:2318-2347`）。
- 角色/群聊信息如何携带：
...
- swipe/branch 保留：JSONL 原样导出完整保留 `swipes`/`swipe_id`/`swipe_info`（含每条 swipe 的 send_date 与 extra）；分支是独立聊天文件（`bookmarks.js:186-243 createBranch`），分支点消息记 `extra.branches` 名单、分支文件 header 记 `chat_metadata.main_chat` 父引用（`bookmarks.js:199,238-241`）——这些字段随 JSONL 原样导出，但导出按文件粒度进行，不打包分支树集合；TXT 只含活动 swipe、无分支概念。swipe 选择器中导出按钮被改造为“创建分支”按钮（`swipe-picker.js:159-174`），不提供按 swipe 导出的功能。
...
- 边界确认：角色卡 PNG/JSON 导出（`#export_button` → `/api/characters/export`，`script.js:11970-12008`）属 Agent 角色类目；SD 资产清单下载（`extensions/assets` `downloadAssetsList`，`index.js:289-313`）是资产列表而非对话导出；自动备份属备份恢复类目。以上均不作为对话导出主体记录。
