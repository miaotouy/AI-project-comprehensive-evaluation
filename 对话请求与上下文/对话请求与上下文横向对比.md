...
| DeepChat | main process 内的自研 Session/Agent runtime 与 typed IPC | transcript、assistant blocks、pending input 和 Provider 流由主进程统一管理，renderer 只维护投影与缓存；另有无头 CLI 控制平面（`src/main/cli/`，server/surface/runService/launcherService 等）与 renderer 共用同一主进程 runtime 和审批 broker |
...
| LobeHub | 自研 ModelRuntime、消息 Store 与 `conversation-flow` | Provider 调用与 DB 消息分层；展示路径由 flow 从消息记录重建；发送入口经重构集中在全局 `conversationLifecycle.ts`（原局部 `sendMessage.ts` 实质逻辑迁入，只剩前置 hook/abort 检查与转发），`/compact` 改由 Command Bus 处理，`/goal` 命令注入 `lobe-goal` 工具 |
...
| --- | --- | --- |
| AIO Hub | 当前活动叶子到根的路径，已被压缩节点遮罩的原消息不进入上下文 | 管道依次处理会话加载、异步结果、正则、转写、世界书、预设注入组装、Recall 检索、变量、Token 限制、格式化和附件解析。世界书不是导入兼容占位：独立编辑器的数据会在 priority 300 处理器中执行关键词/正则、selective、概率、扫描深度、过滤、递归、sticky/cooldown/delay、包含组与位置注入；部分 ST 字段仅保留或降级映射。原 `knowledge-processor` 已被 `recall-processor` 替换，旧 `【kb::...】`/`【knowledge::...】` 只告警，新 `【recall::...】` 为严格协议，并可按 Agent 配置自动注入；最终 Payload 与原始节点数组不同 |
...
| OpenCode | 从 SQLite 重读的全量历史（`MessageV2.toModelMessagesEffect`） | system 拼装：agent.prompt ?? provider 风格 → env/AGENTS.md 指令/MCP 指令/skills → user.system；media 从 tool result 抽离为独立 user 消息；溢出按 `model.limit.input - 20k` 检测，compaction 重排为 [compaction-user, summary-assistant, tail, continue-user]（压缩请求把选中历史**文本序列化**为 `[User]/[Assistant]/[Assistant tool call]/[Tool result]` 前缀文本拼进 summary 请求，结果按 `TOOL_OUTPUT_MAX_CHARS` 截断、compacted 输出标 `[Old tool result content cleared]`，不再经 `toModelMessagesEffect` 的 stripMedia 媒体剥离） |
