...
| --- | --- | --- |
| AIO Hub | 当前活动叶子到根的路径，已被压缩节点遮罩的原消息不进入上下文 | 管道依次处理会话加载、异步结果、正则、转写、世界书、预设注入组装、Recall 检索、变量、Token 限制、格式化和附件解析（原 `knowledge-processor` 已被 `recall-processor` 替换，旧 `【kb::...】`/`【knowledge::...】` 占位符只记录"已废弃"警告、不再执行检索，新 `【recall::...】` 为严格占位符协议，Agent 配置 `autoInjectIfMacroMissing` 时按未引用绑定自动注入到 `context_head` 或 `before_last_user`）；最终 Payload 与原始节点数组不同 |
