...
**路由层**（`src/main/session/routes.ts`）：`chatSendMessageRoute`（:676-691）、`chatSteerActiveTurnRoute`（:693-708）、`chatCancelSubmissionRoute`（:710-718）、`chatStopStreamRoute`（:720-725）、`chatRespondToolInteractionRoute`（:727-734）。send/steer 经 `withSubmissionCancellation`（:129-141）注册 `SubmissionCancellationRegistry`，可在附件准备阶段用 submissionId 取消。
...
- `sendMessage`（:102-111）→ `sendMessageUnderSessionGate`（:113-164）：`normalizeSendMessageInput`（:120）、workdir 同步（:126-132）、`runtime.send`（:133-144）、附件 `needs_user_action` 提前返回（:145-147）、draft 会话提升（:149-154）与标题生成（:155-162）。
- `steerActiveTurn`（:166-206）、`queuePendingInput`（:244-279）、`updateQueuedInput`（:281-290）、`moveQueuedInput`（:292-301）、`convertPendingInputToSteer`/`steerPendingInput`（:303-315）、`resolveBlockedPendingInput`（:317-326）、`deletePendingInput`（:328-331）。
...
- `retryMessage`（:333-375）、`deleteMessage`（:377-381）、`editUserMessage`（:383-390）、`getSessionCompactionState`（:392-399）、`compactSession`（:401-413）、`clearSessionMessages`（:415-420）、`cancelGeneration`（:422-425）、`respondToolInteraction`（:427-437）。
...
- **system prompt、记忆、附件与工具**：`PromptAssemblyService.build`（`src/main/agent/deepchat/runtime/promptAssemblyService.ts:62-79`）经 `buildSystemPromptWithSkills` 组合基础 prompt、技能与工具定义；压缩后的恢复 prompt 由 `createPostCompactionPromptAssembler`（:123-140）注入 checkpoint（`buildContextCheckpoint`）、memory 与 directives。记忆注入发生在每次新 user turn 的 pre-stream 阶段（`turnCoordinator.ts:766-788`）。
- **最终顺序**：`leadingMessages（system + checkpoint）→ 预算内完整 tail turns → 新 user 消息`（`contextBuilder.ts:1673-1688`）。

## 3. 预算、截断、摘要与压缩

- **预算**：`resolveFiniteInputBudget`（`contextBuilder.ts:1372-1381`，contextLength - reserve - extraReserve）与 `resolvePhysicalInputBudget`（:1383-1388）。`buildCacheAwareContextWithMetadata` 内：固定内容（leading + 新 user 消息）超预算时先移除 memory（:1625-1642），再移除 directives（:1644-1661），仍超物理预算抛 `buildCacheAwareOverflowError`（:1663-1670，错误文案 :1390-1403）；历史按完整 tail turns 裁剪（`selectCompleteTailTurns` :1334-1348）。
- **运行时二次防线**：`deepChatLoopRunner` 的 `coreStream`（`src/main/agent/deepchat/runtime/deepChatLoopRunner.ts:701-780`）向 provider attempt 管线提供 `preflight`（:736-743）、`fitStrictRetry`（:744-751，严格重试前再裁剪）与 overflow 错误构造；`recoverRequestContextPressure`（:1221-）在 context-pressure 错误时经 `compactionService.prepareForContextPressureRecovery` 生成压缩 intent 后重试。
- **自动压缩**：每次新 user turn 前 `compactionService.prepareForNextUserTurn`（`src/main/agent/deepchat/runtime/compactionService.ts:311-`，调用点 turnCoordinator.ts:592-608）计算 intent；恢复 turn 用 `prepareForResumeTurn`（compactionService.ts:359-）；intent 落地 `compactionRuntimeCoordinator.apply`（:291-，`createCompactionMessage` 前插/顺移见会话与消息管理笔记 §4）。摘要生成：`generateRollingSummary`（compactionService.ts:774-）、`summarizeBlocks`（:810-）、summary prompt 构造（:965-）与模型调用（:996-）。
...
- `deepChatLoopRunner.run`（`deepChatLoopRunner.ts:381-`）解析 provider model facts（:435-442）、生成设置与 model config（:443-488，`capAgentRequestMaxTokens` 上限），工具目录 `toolCatalog.resolve`（:523-539，`meetTaskContractToolDefinitions` 应用 subagent 任务契约），随后把上下文结果交给 `processStream`（:671）。
...
- **生成侧**：`echo.ts` 双节流——renderer 快照 120ms、DB 落盘 600ms（`src/main/agent/deepchat/runtime/echo.ts:5-6`，`startEcho` :15-77，`cloneBlocksForRenderer` 克隆给 renderer）；事件 `chat.stream.updated`（snapshot，:16-28）。块累积在 `accumulator.accumulate`（`src/main/agent/deepchat/runtime/accumulator.ts:141-`），跨轮次 usage 由 `commitRoundUsage`（:11-）提交。
...
- **停止（界面 → 网络）**：ChatPage `onStop`（见 Chat UI 笔记 §5）→ `ChatService.stopStream`（`chatService.ts:212-`）→ `RunLifecycleCoordinator.cancel`（`runLifecycleCoordinator.ts:258-306`）：`requestGenerationAbort` + `abortDeferredToolCalls` + `cancelProviderPermissions`，未开始流的场景把 pending interaction 块写为 aborted 终态并调度队列（:284-305）。附件准备阶段另有 submission 取消（`chatCancelSubmissionRoute`，`routes.ts:710-718`）。
...
- **入队与认领**：`PendingInputAdmissionCoordinator.queue`（`src/main/agent/deepchat/runtime/pendingInputAdmissionCoordinator.ts:104-139`）按 `shouldClaimImmediately`（`pendingInputPump.ts:295-331`）决定直接 claimed 启动还是入队等待；`sendQueuedMessage`（:141-190）在入队前完成附件准备。`drain`（`pendingInputPump.ts:402-494`）按 steer 优先取下一项（:432-439），restart-held 项不自动启动（:436-438）。
...
- **工具目录**：`deepChatLoopRunner` 的 `toolCatalog`（`deepChatLoopRunner.ts:523-539`）解析 active skills 与工具目录（`resolveActiveSkillNamesForToolProfile` :511-514），subagent 场景经 `meetTaskContractToolDefinitions` 裁剪；system prompt 随技能/工具刷新（`refreshSystemPrompt` :679-698，`promptAssemblyService.build`）。
