...
3. **身份有两层**：进程内 UI 句柄 `sid` 与持久化 `session_key`；压缩轮转生成新 `session_key`，经 `parent_session_id` + `end_reason='compression'` 连成轮转链，跨轮转不变标识是 `list_sessions_rich` 派生的只读字段 `_lineage_root_id`（`hermes_state.py:7464`），桌面端 pin、路由匹配都基于它。
...
- 桌面端 spawn 参数为 `['serve','--host','127.0.0.1','--port','0']`（`electron/backend-command.ts:18-22`）；`HERMES_SERVE_HEADLESS=1` 下 `mount_spa()` 只挂 JSON-RPC/WS/API 面（`hermes_cli/web_server.py:16296`，headless 分支 :16313）。
- 边界：gateway 消息平台接入、cron/kanban/插件体系、Web 端、CLI 交互模式细节本次未调查。

## 端到端聊天主链

一条 text 调用链：桌面 composer → `prompt.submit`（WS）→ `methods_prompt.py` 校验/busy 门控/truncate → `_run_prompt_submit`（`server.py:9707`：记录 turn marker、注册 `_stream` 回调、启动 run 线程）→ `agent.run_conversation`（`conversation_loop.py:1422`：`build_turn_context` 组装上下文、循环调用 Provider，每 token 增量经 `_stream` 回发 `message.delta`）→ `finalize_turn` 持久化 → `message.complete`（含 status/text/usage）。桌面端 delta 进入自适应节流队列（33ms 起、上限 250ms），complete 后合并终态并触发会话列表刷新（300ms 合并）。
...
- **lineage root 跨压缩轮转**：`get_compression_lineage`/`get_compression_tip`（`hermes_state.py:9425/:7004`）只沿 compression 边遍历（上限 100 层）；轮转 `publish_compression_child` 单事务插入子行并写压缩后消息（`hermes_state.py:4670`，`conversation_compression.py`），原地压缩 `archive_and_compact` 同 ID 软归档（`hermes_state.py:8287`）；`_lineage_root_id` 是桌面端 pin/路由匹配的键（`session.ts:246-247`）。
- **编辑 = 截断 + 重发**：无 `session.edit`/`message.edit` RPC（检查范围：方法注册表），编辑经 `prompt.submit` + `truncate_before_user_ordinal`（服务端先 `db.replace_messages` 再改内存历史，要求显式 `confirm_truncate=true`，压缩已结束会话抛错）；无单消息删除，`rewind_to_message` 软删只服务 `/rewind`。
