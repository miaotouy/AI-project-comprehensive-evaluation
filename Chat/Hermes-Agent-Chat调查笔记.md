...
2. **后端 SQLite 是唯一事实源**：会话、消息全部落在 profile 作用域下单一 `state.db`；每会话 JSONL 消息日志已在 spec 002 移除（`gateway/session.py`）。桌面端所有会话原子只是后端真相的缓存，遵循"合并而非覆盖、先乐观后诚实、拒绝乱序回写"的更新原则。
3. **身份有两层**：
   - 进程内 UI 句柄 `sid` 与持久化 `session_key` 分离；
...
- 桌面端以 `['serve','--host','127.0.0.1','--port','0']` 参数 spawn 后端（`electron/backend-command.ts:18-22`）；设置 headless 环境变量时 SPA 挂载只暴露 JSON-RPC/WS/API 面（`hermes_cli/web_server.py:16296`，headless 分支 :16313）。
...
→ methods_prompt.py：校验 / busy 门控 / truncate
→ _run_prompt_submit（server.py:9707）：记录 turn marker、注册 _stream 回调、启动 run 线程
→ agent.run_conversation（conversation_loop.py:1422）：build_turn_context 组装上下文、循环调用 Provider，每 token 增量经 _stream 回发 message.delta
→ finalize_turn 持久化 → message.complete（含 status/text/usage）
```

桌面端 delta 进入自适应节流队列（33ms 起、上限 250ms），complete 后合并终态并触发会话列表刷新（300ms 合并）。

## 核心对象与状态权威

**后端权威**（`state.db`）：会话与消息两表结构如下——

| 表 | 关键字段与语义 |
|---|---|
| `sessions`（`hermes_state_common.py:207-263`） | `id` 主键即持久 `session_key`；`source`；`parent_session_id`（血缘/轮转链唯一关联字段）；`model_config`（内含 `_branched_from`/`_delegate_from` 标记）；`system_prompt_hash`；`archived`/`pinned`；`compression_failure_*` 冷却字段 |
| `messages`（:265-289） | 一行一条消息：`role`、`content`、`tool_call_id`/`tool_calls`、`reasoning_*`、`active`（软删/归档）、`compacted`（压缩归档仍可搜索）、`display_kind`/`display_metadata`、`api_content`（发 API 的字节保真 sidecar，恢复时原样返回） |

- **桌面端权威**：仅原生/OS 性、窗口、流式渲染缓冲、发送前的乐观向量；后端事件投影进本地状态，不做本地合成。状态原子集中在 `store/session.ts`（会话、消息、活动会话 id、busy 标记），per-session 真实状态在 `sessionStateByRuntimeIdRef`。
...
- **lineage root 跨压缩轮转**：lineage/root 查询只沿 compression 边向上遍历（上限 100 层，`hermes_state.py:9425/:7004`）；轮转压缩以单事务插入子行并写压缩后消息，原地压缩则同 ID 软归档（`hermes_state.py:4670`、`:8287`）；`_lineage_root_id` 是桌面端 pin 与路由匹配的键（`session.ts:246-247`）。
- **编辑 = 截断 + 重发**：无单消息编辑 RPC（检查范围：方法注册表），编辑经 `prompt.submit` + 截断到指定用户消息（服务端先替换 DB 行再改内存历史，要求显式 `confirm_truncate=true`，压缩已结束会话抛错）；无单消息删除，`rewind_to_message` 软删只服务 `/rewind`。
- **steer/redirect 分层中断**：`steer` 把文本追加到最后一条 tool 消息 content 尾部（不新增消息、不破坏角色交替），无 tool 可挂时回存为下一轮用户消息（`agent_runtime_helpers.py:3940`）；`redirect` 只打断 model request，不扩散到工具/子代理；每轮开头 `clear_interrupt`。
...
- 后端入口：`tui_gateway/server.py`——`_run_prompt_submit`（:9707）、`_handle_busy_submit`（:7598）、`_emit`/事件帧（:1573）；`methods_prompt.py:67-333`；`methods_session.py`——resume（:306）、branch（:2760）、interrupt（:2942）、steer（:3219）、redirect（:3252）。
- 主循环与上下文：`agent/conversation_loop.py:1422`、`agent/turn_context.py:430`、`agent/turn_finalizer.py:70`。
- 压缩：`agent/context_compressor.py:2906`、`agent/conversation_compression.py:2150`、`agent/native_compaction.py`、`hermes_state.py`——`publish_compression_child`（:4670）、`archive_and_compact`（:8287）。
- 运行与持久化：`run_agent.py`——`_flush_messages_to_session_db_unlocked`（:2010）、`interrupt`/`clear_interrupt`/`redirect`（:3091/:3237/:3328）；`hermes_state.py`——`_lineage_root_id`（:7464）、`append_messages_batch`（:7781）、`replace_messages`（:8187）、`list_sessions_rich`（:7104）、`get_resume_conversations`（:8854）。
- 桌面端：`apps/desktop/src/hermes.ts`、`store/session.ts`（mergeSessionPage :393、sessionPinId :246）、`use-message-stream/index.ts`（completeAssistantMessage、adoptedRunningTurn :656-676）、`use-prompt-actions/submit.ts`（:97）、`rewind.ts`（:122/:140）、`lib/chat-messages.ts`（:13）。
- 崩溃标记：`tui_gateway/turn_marker.py`；子代理：`tools/delegate_tool.py` `_build_child_agent`（:1305）；标题：`agent/title_generator.py`（`derive_title` :250、`apply_instant_title` :474）。
