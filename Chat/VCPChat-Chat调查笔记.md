...
textarea#messageInput（Enter 发送，Shift+Enter 换行）
  ...
  -> electronAPI.sendToVcp -> modules/ipc/chatHandlers.js send-to-vcp（:811-1222）
  ...
  -> 流结束 'end' 事件 -> finalizeStreamedMessage（streamManager.js:2122-2320）写回历史数组并刷新 DOM
...
- `modules/ipc/chatHandlers.js`：`send-to-vcp`（无本地 abort/超时）`:811-1222`，`interrupt-vcp-request`（仅远端信号）`:1226-1271`，`search-topics-by-content` `:364-407`
- `modules/renderer/streamManager.js`：`startStreamingMessage` `:1556-1733`，`finalizeStreamedMessage` `:2122-2320`，群聊不落盘 `:377-396`
...
- `modules/vcpClient.js`：正确但未接入的中断实现（死代码）`:1-544`；`renderer.js`：按钮态与中断触发 `:150-262`，`onVCPStreamEvent` `:523-747`
