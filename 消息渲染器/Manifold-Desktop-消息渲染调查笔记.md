...
assistant Markdown 没有 sanitizer。marked 会保留原始 HTML，返回值直接写入 `innerHTML`（`message-renderer.js:15-17, 102`、`chat-tab.js:42`）。事件处理属性等活动 HTML 因而可能在 WebView 页面上下文执行；通过 `innerHTML` 插入的 `<script>` 元素通常不会自行执行，两种插入方式的行为并不相同。
