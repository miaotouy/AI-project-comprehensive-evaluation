...
- **Part 联合**（schema/src/v1/session.ts:357-370）：`TextPart/SubtaskPart/ReasoningPart/FilePart/ToolPart/StepStartPart/StepFinishPart/SnapshotPart/PatchPart/AgentPart/RetryPart/CompactionPart`。
- 关键结构：TextPart（synthetic/ignored/time/metadata，:102-116）、ReasoningPart（time.start/end，:118-128）、FilePart（mime/filename/url/source，:171-179；source 支持 file/symbol/resource，:166-169）、ToolPart（callID/tool/state，:315-325；ToolState 四态按 status 判别，:259-313）。
- **消息**：`User`（role:"user"）与 `Assistant`（role:"assistant"，含 time.completed/error/parentID/finish/cost/tokens，:332-488）；`WithParts = {info, parts}`（:493-500）。
- 渲染输入：`MessageTimeline` 读 `sync().data.part[msgId]`（message-timeline.tsx:313），`MessagePartProps` 含 part/message/defaultOpen/toolOpen/deferToolContent/virtualizeDiff/onContentRendered 等（message-part.tsx:193-206）。
...
- **行渲染**：`renderTimelineRow` switch（message-timeline.tsx:1118-1273）：UserMessage → `<Message>`（:1182-1188）；AssistantPart → `renderAssistantPartGroup`（:1021-1087，context 组 → `ContextToolGroup`，part 组 → `MessagePart`）；Thinking → `TimelineThinkingRow`（TextShimmer + TextReveal）；Retry → `SessionRetry`；Error → `Card variant="error"`。
- **角色分派**：`Message` 组件按 `message.role` 分派（message-part.tsx:936-963）→ `UserMessageDisplay`（:1181-1392）/ `AssistantMessageDisplay`（:965-1041）。
- **Part 组件注册**：`PART_MAPPING`（:250）：tool → `ToolPartDisplay`（:1534-1633）、compaction → `CompactionPartDisplay`、text → `TextPartDisplay`（:1654-1757）、reasoning → `ReasoningPartDisplay`（:1759-1774）；`registerPartComponent`（:932-934）；未注册类型不渲染。可见性过滤 `renderable`（:711-720）：todowrite 隐藏、question 在 pending/running 时隐藏、text 需非空、reasoning 需开启摘要设置。
...
- **工具组件注册**：`ToolRegistry.register/render`（message-part.tsx:1476-1496），已注册 14 个：read/list/glob/grep/webfetch/websearch/task/shell/edit/write/patch/todowrite/question/skill（apply_patch 经别名 `patch`，:1490）；未注册走 `GenericTool`（basic-tool.tsx:323-343）。
...
- **输出渲染**：list/glob/grep 走 `Markdown`（:1826-1903）；shell `stripAnsi` + `<pre><code>`（:2085-2153）；edit/write/patch 用 `@pierre/diffs` 虚拟化 diff 视图（file.tsx，`VIRTUALIZE_BYTES=500_000` :50）；exa websearch 输出提取 URL（:906-930）；错误 `ToolErrorCard`（tool-error-card.tsx:22-162）。
...
- **附件**：`attached`（url 以 `data:` 开头且非 inline）/`inline`（source.text.start/end）/`kind`（image/file）（message-file.ts:5-15）；`renderAttachments`（:1264-1313）→ `AttachmentCardV2`（v2/components/attachment-card-v2.tsx:5-33）；图片 `ImagePreview`（packages/ui/src/components/image-preview.tsx:10-32）。
...
- **Sanitize**：所有非 code 块 markdown HTML 在插入 DOM 前经 DOMPurify（markdown-cache.tsx:35-38，调用点 markdown.tsx:461）。配置（markdown-cache.tsx:13-20）：`USE_PROFILES:{html:true, mathMl:true}`、`SANITIZE_NAMED_PROPS:true`、`FORBID_TAGS:["style"]`、`FORBID_CONTENTS:["style","script"]`、`ADD_TAGS:["svg","path"]`、`ADD_ATTR:["d","viewBox","preserveAspectRatio","xmlns","target"]`。
...
新增 part 类型需改动：schema `Part` 联合（schema/src/v1/session.ts:357-370）→ SDK 再生成（sdk/js/script/build.ts）→ `PART_MAPPING` 注册（message-part.tsx:250）+ `renderable`（:711-720）→ Timeline 特殊行（timeline-row.ts + rows.ts + renderTimelineRow switch）→ store 过滤（`SKIP_PARTS`：event-reducer.ts:19、server-session.ts:30、sync.tsx:7，当前跳过 patch/step-start/step-finish）→ TUI `PART_MAPPING`（tui/src/routes/session/index.tsx:1564-1568）。新增工具则 `ToolRegistry.register`（:1484-1491）+ TUI `toolDisplay` 分派。

**TUI 侧**（packages/tui）：消息壳 `routes/session/index.tsx`；part 映射 `{text, tool, reasoning}`（:1564-1568）；markdown 用 opentui 内置 `<markdown>`（streaming、tableOptions、conceal，:1685-1694），代码块用 tree-sitter wasm（parsers-config.ts:2）；语法样式 `context/theme.tsx:271-272` + `theme/index.ts:66-79`（映射 ANSI :441-455）；工具状态 `ToolPart`（:1702-1782）+ `InlineTool`（:1829-1905）+ `collapseToolOutput`（3 行截断）；导出文本 `util/transcript.ts:26-112`。
