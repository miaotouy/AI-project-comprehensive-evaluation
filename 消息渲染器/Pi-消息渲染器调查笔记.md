...
> 调查对象：`../../pi`（重点 `packages/tui/`、`packages/coding-agent/src/modes/interactive/components/`）
>
> 调查更新日期：2026-08-10
>
...
1. **输入模型是完整消息对象**：`AgentSessionEvent`（`message_start/update/end`、`tool_execution_*`、`bash_execution_update` 等）携带 `AgentMessage` 或增量事件（`core/agent-session.ts:141-183`）；`AssistantMessageEvent` 细粒度事件（text/thinking/toolcall 的 start/delta/end）随 `message_update` 透传给 UI（`interactive-mode.ts:3114-3146`）。
...
4. **消息壳层按 role/类型分派**：`addMessageToChat` 按 user/assistant/bashExecution/custom/compactionSummary/branchSummary 分支建组件（`interactive-mode.ts:3422-3510`）；assistant 组件按 content 块顺序渲染 text/thinking（toolCall 块由 interactive-mode 另建 `ToolExecutionComponent`），并按 stopReason 追加截断/中止/错误状态文本（`components/assistant-message.ts:89-196`）。
5. **列表是普通 Container，无虚拟化**：聊天区是 `chatContainer: Container`（`interactive-mode.ts:534`），流式时在末尾追加组件、重建内容，滚动交给 TUI 主屏幕整帧渲染；`packages/tui` 提供 `ScrollView`（滚动条/动画）组件，但聊天列表未使用虚拟化。
...
AgentSession 事件 (message_start/update/end, tool_execution_*, bash_execution_update)
  -> InteractiveMode._handleSessionEvent (interactive-mode.ts:3090-3361)
...
- **消息**：`AgentMessage`（user/assistant/toolResult + custom/bashExecution/compactionSummary/branchSummary，`core/messages.ts:29-67`）；assistant 内容为 `(TextContent | ThinkingContent | ToolCall)[]`（`packages/ai/src/types.ts:337-365`）。
- **事件携带**：`message_update` 携带 `assistantMessageEvent`（text_delta/thinking_delta/toolcall_delta 等，`types.ts:515-531`），UI 不消费 delta 本身而是用携带的 `partial` 完整消息重建（`interactive-mode.ts:3114-3146`）。
- **角色分派点**：`addMessageToChat` 的 switch（`interactive-mode.ts:3422-3510`）与 `message_start` 分支（`interactive-mode.ts:3090-3111`）是唯一分派入口；`toolCall` 块在 `message_update` 时同步建 `ToolExecutionComponent`（`interactive-mode.ts:3119-3144`）。
...
- **工具参数流式**：toolcall_delta 逐步 `updateArgs`（`interactive-mode.ts:3137-3142`）；`tool_execution_update` 以部分结果刷新 `ToolExecutionComponent`（`interactive-mode.ts:3217-3224`），`maybeConvertImagesForKitty` 对非 PNG 图片异步转码后重绘（`tool-execution.ts:178-199`）。
- **收口**：`message_end` 时 `updateContent(final, false)`，按 stopReason 补状态文本（aborted/error/length），中止或错误时把原因写进所有挂起工具组件（`interactive-mode.ts:3149-3187`）；`agent_end` 清理流式组件与挂起工具表（`interactive-mode.ts:3236-3249`）。
- **bash 流式**：`bash_execution_update` 事件经 `BashExecutionComponent.appendOutput` 增量追加（`interactive-mode.ts:3189-3191` 注释与 `components/bash-execution.ts`）。
- **节流**：UI 只调用 `requestRender`，实际帧合并/降频在 `tui.ts`（`MIN_RENDER_INTERVAL_MS`，强制 `force=true` 可立即渲染）。

## 3. 消息列表、窗口化与滚动

- **列表结构**：聊天区是普通 `Container`，无分页/虚拟化/窗口化；流式期间组件持续追加（`interactive-mode.ts:534`）。会话列表（`/resume`、`/tree`）用 `SelectList`/`TreeSelectorComponent`，是选择器而非渲染大列表。
- **滚动与高度**：整屏组件树由 `renderLayoutFrame(root, width, height, requestRender)` 布局（`tui-alt-screen.ts:831`）；滚动行为由 TUI 主屏幕管理（输入区与聊天区共存，`tui-main-screen.ts`）。`packages/tui` 提供通用 `ScrollView`（滚动条、鼠标滚轮与 API 滚动，`components/scroll-view.ts`；无拖拽），本次未在聊天列表中找到虚拟化使用。
- **分支切换**：`/tree` 提交导航走 `chatContainer.clear() + renderInitialMessages()`（interactive-mode.ts:5015-5016，`renderInitialMessages` 定义 :3662）；`rebuildChatFromMessages`（:3713）用于压缩结束、thinking 切换、设置变更与会话重载场景。
...
- **系统/扩展消息**：`CompactionSummaryMessageComponent`、`BranchSummaryMessageComponent`（可折叠）、`CustomMessageComponent`/`CustomEntryComponent`（扩展渲染器）、`SkillInvocationMessageComponent`（可折叠的 `<skill>` 块，`interactive-mode.ts:3472-3479`）。
...
- **LaTeX**：`latex.ts` 将 LaTeX 转 Unicode 文本（`renderLatex` 选项默认开）。
...
- **自定义节点**：扩展注册 `getMessageRenderer(customType)`/`getEntryRenderer(customType)` 返回组件工厂（`getEntryRenderer` 在 addCustomEntryToChat，interactive-mode.ts:3401；`getMessageRenderer` 在 :3440）；bash 执行、压缩摘要、分支摘要都是独立节点类型。
...
- **复制/折叠/重试**：`/copy` 复制最后一条助手消息；工具/摘要组件支持展开收起（`setExpanded`，`interactive-mode.ts:3134`）；自动重试与压缩有状态指示器与 Esc 中止（`interactive-mode.ts:3308-3334`）。
- **键盘**：全局按键系统（`keybindings.ts`、`packages/tui/src/keybindings.ts`），`/hotkeys` 查看；编辑器支持 IME 与硬件光标选项（`settings-manager.ts:128`）。
...
- `packages/coding-agent/src/modes/interactive/interactive-mode.ts:3090-3234`：事件→组件
