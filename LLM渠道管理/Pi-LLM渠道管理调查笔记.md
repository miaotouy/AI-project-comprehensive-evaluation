...
1. **Provider 是代码注册项，不是可多实例化的用户实体。** 每个 Provider 是 `packages/ai/src/models.ts` 中 `Provider` 接口的一个对象，固定 `id`；同一 Provider 类型在仓库内只有一份内置注册（`providers/all.ts:89-121`），用户通过 `models.json` 的 `providers` 覆盖字段和扩展注册，不能创建同一 Provider 的多个并列实例。Radius 网关通过 `providers/radius.ts` 的 `radiusProvider({ id })` 工厂支持按配置生成新 id，是唯一的多实例入口。
...
- **Radius 例外**：`providers/radius.ts:20-34` 的 `radiusProvider(options)` 以传入 `id` 构造 Provider，`coding-agent` 在 `model-runtime.ts:219-230` 为每个 `models.json` 中 `oauth === "radius"` 且有 baseUrl 的配置生成一个新 id 的 Radius Provider；`providers/radius-config.ts:4` 默认网关为 `https://radius.pi.dev`。
...
- **订阅制标记**：`OAuthAuth` 新增 `isSubscription` 标志，GitHub Copilot、Kimi Code、OpenAI Codex、xAI 的 OAuth 均标记为订阅制（`providers/github-copilot.ts:16`、`kimi-coding.ts:17`、`openai-codex.ts:13-17`、`xai.ts:17`）；`ModelRuntime.isUsingSubscription`（`model-runtime.ts:462`）据此判断，footer 只对已知订阅制显示 `(sub)`。
...
- **默认与用户合并**：`composeModelProvider`（`provider-composer.ts:420-520`）按“内置 → models.json → 扩展”顺序合并：`applyModelsJson` 先改内置模型（baseUrl、compat、新增/替换模型），`applyExtension` 再套扩展模型列表，最后 `modelOverrides` 做最顶层单模型字段覆盖（`provider-composer.ts:434-443`）。`recomposeProvider`（`model-runtime.ts:241-263`）在无覆盖层时直接使用内置 Provider 原对象。
...
- **模型元数据**：`Model` 接口（`types.ts:794-813`）包含 `id/name/api/provider/baseUrl/reasoning/thinkingLevelMap/input/cost/contextWindow/maxTokens/samplingParams/headers/compat`。价格是 `ModelCost { input, output, cacheRead, cacheWrite, tiers? }`（美元/百万 token，`types.ts:776-790`），成本由 `calculateCost` 结合用量计算（`models.ts:878-898`，含 Anthropic 1h cache write 2 倍规则）。
...
- **API 模块**：`packages/ai/src/api/` 下 11 个协议实现：`openai-completions`、`openai-responses`、`openai-codex-responses`、`azure-openai-responses`、`anthropic-messages`、`bedrock-converse-stream`、`google-generative-ai`、`google-vertex`、`mistral-conversations`、`pi-messages`、`cloudflare-gateway-binding`（AI Gateway 经 Cloudflare AI binding 传输，#7901）（`types.ts:17-29`）。每个模块导出 `stream`/`streamSimple`（`ProviderStreams` 契约，`types.ts:267-276`），lazy 包装用于 tree-shaking（`api/lazy.ts`）。Mistral 由 SDK 传输改为原生 HTTP 流（`api/mistral-conversations.ts`，去掉生成客户端与 schema 运行时开销，#9dd90a4）。
...
- **兼容探测**：OpenAI-compatible 的 `detectCompat`（`api/openai-completions.ts:1443`）按 provider 名 + baseUrl 特征（openrouter.ai、deepseek.com、api.z.ai、api.moonshot、gateway.ai.cloudflare.com、chutes.ai 等）自动决定 developer role、thinking 格式、max_tokens 字段、cache 控制等（`api/openai-completions.ts:1439-1489`），`model.compat` 可显式覆盖（`types.ts:545-597`）。DeepSeek 的 baseUrl 探测改为大小写不敏感、且发送 `max_tokens`（`openai-completions.ts`，#7933/#7930）；Fireworks GLM 走 Anthropic Messages 兼容端点并修正 prompt caching（`providers/fireworks.ts` 用 `anthropicMessagesApi`，#7676）；OpenAI Responses 侧新增 `supportsAdditionalTools` 兼容开关（延迟工具经 `additional_tools` 注入，#e47b8e3）。z.ai 仅走 openai-completions（zai.ts:6-14），`api/anthropic-messages.ts:1228` 提及 z.ai 的注释与当前实现不一致。
...
- **Assistant 消息层重试**：`retryAssistantCall`（`ai/src/utils/retry.ts:162-211`）对 stopReason=error 的消息按错误文本分类（`isRetryableAssistantError`，正则匹配 overloaded/rate limit/429/5xx/网络/websocket 关闭/流提前结束等；"exceeded request buffer limit while retrying upstream" 可重试模式，`retry.ts:44`；quota/billing 类不可重试，`retry.ts:7-89`），指数退避 `baseDelayMs * 2^(n-1)`（默认 2000ms、最多 3 次，`settings-manager.ts:818-822`），abort 归一化为 aborted 消息。
...
- **连接检测**：`Models.checkAuth`（`models.ts:485-520`）只检查凭据完整性（存储/环境/命令源），不发真实请求；`models.json` 内嵌 key 的检查只确认环境变量引用存在（provider-composer.ts:329-331），命令值不执行也不验证（:326-327，与 types.ts:73-75 的 list 约束一致）。TUI 的 `/login` 登录流程中 OAuth 会真实换取 token，但无独立的“连接测试”入口。CLI `pi auth check`（`cli/auth-command.ts`、`cli/auth-check.ts`）：复用 `ModelRuntime.checkAuth` 检查 Provider 凭据就绪度（`ready/not_ready/invalid`，退出码 0/1/2），支持 `--json`/`--credentials`/`--no-refresh`，全程不发真实请求（`auth-check.ts:39-71`）。
