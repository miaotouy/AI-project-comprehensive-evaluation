...
当前 Cohere 分支还有代码顺序问题：通用路径先发送原始响应，随后才把 `data.models` 转成 `{id, ...model}` 形式的 `data.data`。这个转换不会进入已经发送的响应，前端能否正确加载 Cohere 模型取决于原始响应是否恰好符合后续预期。
...
这套策略能覆盖缺少元数据的新模型，但 fallback 可能高估未知模型。高估会让客户端构建超长 prompt，最终由上游返回 context error；错误不会反向修正本地模型元数据。
