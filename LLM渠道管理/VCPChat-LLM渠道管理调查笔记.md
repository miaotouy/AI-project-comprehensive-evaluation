...
实际字段由 `modelConfig` 展开，工具、上下文清理和 `vcpchatExtensions` 可能继续增补内容。协议入口兼容 OpenAI Chat Completions，但不是纯粹只发 OpenAI 标准字段。
...
[`modules/vcpClient.js`](../../VCPChat/modules/vcpClient.js) 另有一个统一请求模块，包含 `AbortController` 和 300 秒定时中断，流式清理也更完整。然而全仓库对 `vcpClient` 的命中只有该文件自身，没有 `require()` 或 `import` 把它接入当前主流程。

实际生产路径是 `main.js` 注册的 [`modules/ipc/chatHandlers.js`](../../VCPChat/modules/ipc/chatHandlers.js) 中 `send-to-vcp` handler。该 handler 的 `fetch` 没有传 `signal`，所以不能把未接线模块的 300 秒超时算作当前客户端能力。
