...
会话单位只有一种持久化对象 `Chat`（对应 `chat` 表），`temporary:`/`local:` 前缀是临时会话（不落库，前端发送前占位，见 `utils/chat_id.py:4-20`），`channel:` 前缀是频道会话（走另一套 channel 消息体系）。会话 ID 由路由侧 `str(uuid4())` 生成（`chats.py:769`）；消息 ID 由前端 `uuidv4()` 生成（`Chat.svelte:2509`），服务端按 `{chat_id}-{message_id}` 拼复合主键。
...
- `chat_list_order`（46-67 行）支持 `updated_at` / `title` / `unread_updated_at` 排序；未读排序以 `ChatMessage.done=False` 的 assistant 子查询加 `updated_at > last_read_at` 差值计算（52-67 行）；
...
- `get_messages_map_by_chat_id`（331-392 行）把行还原为与 `chat.history.messages` 同构的字典（`parent_id→parentId`、`model_id→model`、`created_at→timestamp` 等映射，321-327 行），并重建 `childrenIds`、回填 `info.usage`；无行时返回 None，调用方回退旧版 JSON blob；
...
- **频道消息**：`models/messages.py`（39-56 行），`channel_id`/`reply_to_id`/`parent_id`/`is_pinned`/`content`/`data`/`meta`，仅服务 `channel:` 会话。
...
- **索引**：`chat` 表 `folder_id`、`user_id+pinned`、`user_id+archived`、`updated_at+user_id` 复合索引（95-101 行）；`chat_message` 表 `(chat_id, parent_id)`、`(model_id, created_at)`、`(user_id, created_at)` 索引（168-172 行）；
- **事件源**：所有变更经 `publish_event` 发审计/Webhook 事件（`events.py:161-221` 定义 `EVENTS.CHAT_CREATED/CHAT_FINISHED/CHAT_FAILED/CHAT_UPDATED/CHAT_DELETED/CHAT_ARCHIVED/CHAT_SHARED/...` 与 `MESSAGE_CREATED/MESSAGE_UPDATED/MESSAGE_DELETED`），与 Socket.IO 前端事件是两套通道。
...
- **编辑**：`POST /{id}/messages/{message_id}` 只改目标消息的 `content`（`MessageForm`，1402-1456 行）；权限校验 `chat.user_id != user.id and user.role != 'admin'`（1423-1427 行，删除端点同构 1484-1488 行）；
- **分支（fork）**：`POST /{id}/fork` 调 `build_fork_history`（`utils/chat_fork.py:4-41`）沿 `parentId` 从源消息一路回溯到根、deepcopy 后重建单向 `childrenIds` 链，生成**新会话**（`originalChatId`/`branchPointMessageId`/`forked_from` 记录来源，1730-1734 行），不是原会话内切分支；
...
- GET `/list`（`chats.py:220-259`）：分页 60 条/页（233-235 行），支持 `include_pinned`/`include_folders`/`sort_by`/`sort_dir`；`add_active_state_to_chat_list` 依据 `ACTIVE_CHAT_GAP_SECONDS` 标记活跃；
- 未读：`POST /read`（262 行）批量置已读并返回 `folder_unread_counts`；`POST /{id}/unread`（2092 行）把 `last_read_at` 置 0；前端读消息经 socket `events:chat`（`socket/main.py:534-579`）更新 `last_read_at` 并向房间广播 `chat:list`；
- 搜索 `GET /search`（846 行）：`Chats.get_chats_by_user_id_and_search_text`（`models/chats.py:1735`）支持 `tag:`/`folder:`/`pinned:`/`archived:`/`shared:` 前缀过滤（1757-1796 行），文本部分对 **title ilike 或 messages 内容 JSON 搜索**（SQLite `json_each` 1817-1840 行，PostgreSQL 走同构条件）；`tag:` 前缀无结果时自动删标签（875-882 行）；命中片段 `chat_search_snippet` 随行返回（871 行）；
...
- **流式增量落库**：生成中 `update_db=True` 的事件按类型写表（`socket/main.py:997-1092`）：`status`→status_history 追加、`message`→content 追加、`replace`→content 覆盖、`embeds`/`files`→按 replace 标志追加或覆盖、`source`/`citation`→sources 追加；
...
- **Schema 版本管理**：Alembic，版本位于 `backend/open_webui/migrations/versions/`（55 个版本），其中 `8452d01d26d7_add_chat_message_table`、`4c5ce3d2f27f_add_context_summary_to_chat_message`、`1af9b942657b_migrate_tags`、`242a2047eae0_update_chat_table` 等直接涉及 chat/chat_message 表；
- **兼容路径**：消息表无行即视为旧版数据，回退 JSON blob 并回填（第 2 节）；`DB_TO_JSON_KEY_MAP`/`EXCLUDED_COLUMNS`（`chat_messages.py:321-329`）保证行→字典转换对未知字段宽容；
- **导入导出**：`POST /import`（788 行）与 `import_chats`（`models/chats.py:534-589`）支持整包导入并双写消息表，导入时清理悬空 folder_id（541-554 行）；统计导出 `GET /stats/usage`（279 行）、`GET /stats/export`（581/627 行）；
- **保留策略**：本次未找到自动过期回收或定时清理 chat 的代码（检查范围：`routers/chats.py` 全部端点与 `Chats` 表操作方法）；批量清理只存在于手动 `DELETE /`、`POST /archive/all`（1093 行）、`POST /unarchive/all`（1108 行）。

## 8. Agent、模型、知识库与附件绑定

- **会话级**：`folder_id`、`variables`、`meta`（tags、internal、forked_from 等）、`tasks` 快照；
- **消息级**：`model_id`（生成模型）、`files`（附件快照）、`sources`（引用）、`embeds`、`output`（工具调用与推理的 OR 输出项）、`usage`；`ChatFile` 表另存 `chat_id+message_id+file_id` 关系（139-152 行）；
- **任务级（不持久化在消息上）**：`tool_ids`/`skill_ids`/`filter_ids`/`features` 只随生成请求传递，`chat_message` 表列中不存在这些字段（表结构 128-172 行）；工具调用过程以 `output`/`meta` 内容留存（`_extract_tool_names` 扫描这些字段即为此类统计设计，89-120 行）；
...
- **消息树用 `modelIdx` 保留多模型并行的列序**：side-by-side 是内建数据语义而非插件（`chat_messages.py` 的 `modelIdx` 字段随消息内容写入，`main.py:1287-1290` 保留）；
