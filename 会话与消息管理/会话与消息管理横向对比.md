...
| LobeHub | Agent/topic/thread（多维 `messageMapKey`） | 服务端 + 本地双份缓存，双层 Store 各自独立解析 | 双层 parse 问题见设计取舍 |
...
- **Pi**：搜索是选择器内的 `id+名称+全部消息文本+cwd` token/正则匹配，结果是会话级命中；无消息级持久化索引。
...
- **Chatbox**：IndexedDB 有意不做 version 升级，代码注释所述的"重试兜底"也未找到实现。
...
- **LobeHub**：`doctor/diagnose.ts` 用于检测和修复读取器无法完整渲染的消息树；`reconcileAssistantToolLinks` 用于修复乐观更新在 step 边界丢失工具引用的问题。
...
- **VCPChat**：群聊历史写盘没有文件锁，存在并发覆盖丢消息的可能，尚未构造场景验证；未读自动判定要求整个历史仅有一条非系统 assistant 消息，因此多轮对话不会触发。
