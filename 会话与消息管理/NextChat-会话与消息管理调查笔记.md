...
- `useChatStore` 以 `ChatSession[]` 保存会话、消息、Mask 副本、记忆摘要与上下文截断索引；内存数组是权威源，IndexedDB 是持久化副本，异常时回退 localStorage（persist key `chat-next-web-store`，version `3.3`）。
...
- 消息编辑、删除、置顶、重试都是对 `session.messages` / `mask.context` 的就地变更：编辑改 `content`、删除按 id 过滤、置顶 push 进 `mask.context`、重试删除原 user+bot 消息对后重新发送。
...
- `ChatMessage` 继承 `RequestMessage`，`content` 可以是字符串，也可以是 `text + image_url` 多模态数组（`app/client/api.ts:36-52`）。
- `tools` 是 `ChatMessageTool[]`（`app/store/chat.ts:44-55`），保存当前 assistant 消息的工具调用状态；工具工作流与最终正文共享同一消息容器，不单独建消息节点。
- `createMessage` 用 `nanoid` 生成 id（`app/store/chat.ts:68-76`）；`createEmptySession` 建立空会话、空记忆和空 Mask（`app/store/chat.ts:104-120`）。`BOT_HELLO` 是内置问候消息（`app/store/chat.ts:99-102`）。
- `stat` 只有 `charCount` 实际累加：`updateStat` 只做 `charCount += content.length`，并有 TODO 注释未更新 word/chat 数（`app/store/chat.ts:799-804`）。
- Mask 是完整副本对象（`app/store/mask.ts:9-23`），每个会话持有独立的一份；`createEmptyMask` 默认 `syncGlobalConfig: true`（`app/store/mask.ts:35-47`）。

分支模型：`forkSession()` 深拷贝当前消息并为每条消息生成新 id，同时复制 Mask/modelConfig，新会话插到数组头部（`app/store/chat.ts:243-267`）——fork 是"新会话深拷贝"，不是消息树指针。消息级分支（同一位置的平行版本）与版本指针：本次未找到（`ChatSession` schema 无 active pointer / parent 字段，store 无对应方法，检查范围为 `app/store/chat.ts` 全文件）。
...
3. 注入 `lastUpdateTime`、`_hasHydrated`、`markUpdate`/`update`（`update` 用 `deepClone` 整体克隆后写回并刷新时间戳）与 `setHasHydrated`（`app/utils/store.ts:15-22, 56-71`）。
...
2. `setItem` 先解析 JSON，`_hasHydrated` 为假则跳过写入；IndexedDB 失败时写 localStorage（`app/utils/indexedDB-storage.ts:17-28`）；
3. `removeItem`/`clear` 同样有 localStorage 回退（`app/utils/indexedDB-storage.ts:30-44`）。

Chat store 的 persist 配置位于 `app/store/chat.ts:861-930`：name 为 `StoreKey.Chat`（`chat-next-web-store`，`app/constant.ts:90-101`），当前 version `3.3`（`app/store/chat.ts:863`），并带 `migrate`（见 §7）。

事实源层级：会话列表和消息没有独立索引或分页存储——全部会话是 `ChatSession[]` 内存数组，整份数组序列化进持久层；Mask、Prompt、Plugin、Access、Config 各自独立 store 与独立 persist key（`app/store/mask.ts:49`、`app/store/prompt.ts:51`、`app/store/plugin.ts:230-232`、`app/store/access.ts:156`、`app/store/config.ts:164`）。服务端不保存会话，`/api/config` 只做能力探测（见对话请求与上下文笔记 §4）。

## 3. 创建、切换、归档、删除与恢复

- `newSession(mask?)`：新建空会话插到头部；传入 Mask 时把全局 `modelConfig` 与 Mask 的 `modelConfig` 合并，并设置 topic 为 Mask 名称（`app/store/chat.ts:307-328`）。
- `currentSession()`：读取 `currentSessionIndex` 对应的会话，越界时钳制索引并回写（`app/store/chat.ts:380-392`）；`selectSession`/`nextSession` 切换（`app/store/chat.ts:276-280, 330-335`）。
...
- `clearSessions()`：清空为单个空会话（`app/store/chat.ts:269-274`）；`resetSession()`：清空消息和 `memoryPrompt`，保留会话与 Mask（`app/store/chat.ts:654-659`）；`clearAllData()`：清空 IndexedDB 与 localStorage 后 `location.reload()`（`app/store/chat.ts:815-819`）。
...
- 重命名：无独立方法；会话 `topic` 字段可被直接修改（编辑入口在 Chat UI 笔记 §6，`app/components/chat.tsx:850-912` 的 `EditMessageModal` 与 `app/store/chat.ts:805-814` 的 `updateTargetSession`）。
- 异常恢复：`Chat` 组件挂载/会话切换时把超时（`REQUEST_TIMEOUT_MS = 60000`，`app/constant.ts:115`）仍带 `streaming` 标记的消息强制结束，空内容标记为 `isError` 并写入 "empty response" 错误对象（`app/components/chat.tsx:1148-1175`）——这是崩溃/刷新后残留半截消息的清理路径。
...
- **编辑**：单条消息编辑把新文本写回原消息的 `content`（含图片消息重组多模态数组），查找范围是 `mask.context.concat(messages)`（入口 `app/components/chat.tsx:1814-1848`）；批量编辑经 `EditMessageModal` 替换整个 `session.messages` 数组与 `session.topic`（`app/components/chat.tsx:850-912`）。
...
- **clearContextIndex**：把发送起点抬高，在不删除 UI 历史的情况下停止发送更早消息与摘要（数据语义；截断执行见对话请求与上下文笔记 §2）。重置为 `undefined` 恢复（`app/store/chat.ts:545`、`app/components/chat.tsx:382-402` 的 `ClearContextDivider` 与 `:661-674` 的 Clear 按钮）。
...
- 消息分页接口：窗口大小 `CHAT_PAGE_SIZE = 15`（`app/constant.ts:914`）；初始索引定位到末尾窗口（`app/components/chat.tsx:1386-1388`）；每次渲染 `msgRenderIndex` 起最多 `3 * CHAT_PAGE_SIZE` 条（`app/components/chat.tsx:1396-1402`）；滚动触及顶部/底部边缘时索引 ±15 移动，超出范围钳制（`app/components/chat.tsx:1390-1394, 1404-1424`）；`scrollToBottom` 恢复末尾窗口（`app/components/chat.tsx:1426-1429`）。这是数据侧分页窗口，消息仍完整保存在 `session.messages` 中（DOM 窗口行为见消息渲染器笔记）。
- 搜索：`SearchChatPage` 跨会话全文搜索（`app/components/search-chat.tsx:18-166`）：对每个会话的每条消息做子串匹配（`indexOf`，无索引、无分词），命中处截取前后各 35 字符拼成片段（`app/components/search-chat.tsx:30-68`）；结果按片段总长降序（`search-chat.tsx:64-67`）；点击结果 `navigate(Chat) + selectSession(index)` 只定位到会话，不定位到具体消息行（`search-chat.tsx:136-161`）。会话内搜索：本次未找到。
- 命中定位：搜索面板无消息行级高亮/跳转（结果项只展示会话主题与文本片段，静态代码未发现命中消息 id 回传）。

## 6. 缓存、一致性、多窗口与并发写入

- 写入时机：store 每次 `set` 都触发 persist 写入（Zustand persist 默认行为，`createPersistStore` 未配置节流/partialize，`app/utils/store.ts:29-78`）；流式期间每个动画帧的 `onUpdate → updateTargetSession → set` 都会产生一次写入（调用链见对话请求与上下文笔记 §5）。
- hydration 闸门：`setItem` 在 `_hasHydrated` 为假时跳过（`app/utils/indexedDB-storage.ts:19-23`），避免 hydration 前覆盖。
- 本地/远端合并：`app/utils/sync.ts:33-119` 为 Chat、Access、Config、Mask、Prompt 定义 getter/setter 与合并策略：
...
- 其他 store 版本：Mask `3.1`（`app/store/mask.ts:117-136`）、Sync `1.2`（`app/store/sync.ts:128-148`）、Config `4.1`（`app/store/config.ts:198`）、Plugin `1`（`app/store/plugin.ts:232`）。
- 导入导出：`useSyncStore.export()` 下载 `Backup-*.json`（`app/store/sync.ts:58-68`）；`import()` 读取 JSON，经 `mergeAppState` 合并进本地各 store 后 `location.reload()`（`app/store/sync.ts:70-83`）。
- 云端同步：`useSyncStore.sync()` 先取远端状态再合并回写（`app/store/sync.ts:91-120`），WebDAV/Upstash 客户端由 `createSyncClient` 选择（`app/utils/cloud/index.ts:4-28`）；WebDAV 走服务端代理，固定读写远端 `backup.json`，endpoint 有域名白名单校验（`app/api/webdav/[...path]/route.ts`）。
- 保留策略：摘要流程只写 `memoryPrompt` 与 `lastSummarizeIndex`，从不删除原始消息（`app/store/chat.ts:727-796`；失败细节见对话请求与上下文笔记 §6）；本地始终保留完整历史。

## 8. Agent、模型、知识库与附件绑定

- 绑定粒度是会话级：每个 `ChatSession` 持有完整 `Mask` 副本（模型配置、context 预置消息、`plugin` id 数组、记忆开关都在 Mask 上，`app/store/mask.ts:9-23`）；`newSession(mask)` 时合并全局模型配置（§3）；`syncGlobalConfig=true` 时 Chat 页面每次挂载前把全局 `modelConfig` 覆盖到会话（`app/components/chat.tsx:1168-1172`）。
- 消息级绑定：`ChatMessage` 保存 `model`、`tools`（工具调用与结果）、`audio_url`、`isMcpResponse` 等字段（§1）；图片以多模态数组存进消息 `content`。
...
- **摘要不是严格压缩协议**：摘要流式过程中 `onUpdate` 直接把部分文本写入 `memoryPrompt`，无质量校验或回滚；`onFinish` 仅在响应 200 时提交 `lastSummarizeIndex`（`app/store/chat.ts:780-791`）。
