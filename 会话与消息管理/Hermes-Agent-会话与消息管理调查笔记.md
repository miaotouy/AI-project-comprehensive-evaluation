...
- **身份有两层**。进程内 UI 句柄 `sid`（`uuid4().hex[:8]`，`methods_session.py:16`）与持久化 `session_key`（`YYYYMMDD_HHMMSS_uuid4.hex[:6]`，`server.py:6797`，响应里叫 `stored_session_id`，`methods_session.py:131`）。压缩轮转生成新 `session_key`，通过 `parent_session_id` + `end_reason='compression'` 连成轮转链；“跨轮转不变标识”不存在独立列，而是 `list_sessions_rich` 派生的只读字段 `_lineage_root_id`（`hermes_state.py:7464`），桌面端 pin、路由匹配、草稿作用域都基于它（`session.ts:246-247`）。
...
- `id` 主键即持久 `session_key`；`source`（`cli`/`tui`/`telegram`/`subagent`…，`:209`）；`parent_session_id`（`:222`，外键 `:262-263`）是血缘/轮转链的唯一关联字段；`started_at/ended_at/end_reason`（`:223-225`）。
- `title`/`title_source`（`:244-245`）承载标题 provenance；`model_config` JSON（`:219`）内含 `_branched_from`/`_delegate_from` 标记；`system_prompt`/`system_prompt_hash`（`:220-221`，v25 起提示词去重到 `system_prompts` 表 `:202-205`）。
- `archived`/`pinned` 软状态（`:259-260`）；`message_count`/`tool_call_count`/token 用量聚合列（`:226-243`）；`compression_failure_cooldown_until` 等压缩失败冷却字段（`:253-256`）；`cwd`/`git_branch`/`git_repo_root`（`:233-235`）；`last_activity_at`（`:246`）。

会话 ID 生成统一为时间戳 + 6 位随机 hex：`tui_gateway/server.py:6797` `_new_session_key`（`%Y%m%d_%H%M%S` + `uuid4().hex[:6]`）、`agent/agent_init.py:1512-1519`、`agent/conversation_compression.py:3346-3347`；gateway 消息平台用 8 位 hex（`gateway/session.py:2711, 3218`）。TUI/桌面网关另有进程内句柄 `sid = uuid.uuid4().hex[:8]`（`methods_session.py:16`），返回给前端的是它，DB 行主键是 `stored_session_id`。
...
链遍历工具：`get_compression_lineage`（`hermes_state.py:9425`）、`get_compression_tip`（`:7004`，只沿 compression 边）、`resolve_resume_session_id`（`:8566`）、`_session_lineage_root_to_tip`（`:9044`，`range(100)` 上限 100 层，`:9052`）。

压缩轮转的数据语义：`agent/conversation_compression.py`——生成新 session_id → `publish_compression_child` 单事务插入子行（继承 cwd/git/profile/gateway origin、携带标题与压缩后消息）并写压缩后消息、父行 `end_reason='compression'`（`hermes_state.py:4670` 起）→ `agent.session_id` 切换 → flush 基线重置。原地压缩 `archive_and_compact` 不轮转 ID，软归档 `active=0, compacted=1`（`hermes_state.py:8287`），同一会话 ID 贯穿一生。压缩的触发与预算语义在对话请求与上下文笔记 §3。
...
索引：`idx_messages_session(session_id, timestamp)`（`:359`）与 `idx_messages_session_id(session_id, id)`（`:360`）、assistant 且 tool_calls 非空的专用部分索引（`:366-368`）、`idx_messages_session_active(session_id, active, timestamp)`（`hermes_state_common.py:382-383` 延迟索引段）。工具结果与 assistant 调用靠 `tool_call_id` 关联，无独立子表。附件无表：图片经 `@image:` 指令或 vision 预分析转文本，落库时多模态图片部分被压成 `[screenshot]` 占位（`run_agent.py:2214-2227`，`agent/tool_dispatch_helpers.py:515-526` 同样处理）。

桌面端模型 `ChatMessage`（`apps/desktop/src/lib/chat-messages.ts:13-32`）：`id`、`role`、`parts`（assistant-ui part 数组：text/reasoning/tool-call/图片）、`timestamp`、`pending`、`error`、`branchGroupId`、`hidden`、`interim`、`attachmentRefs`、`rowId`（对应后端 durable `messages.id`）、`reactions`。reasoning 是 part 类型而非顶层字段（`:133-134`）；模型名不在消息上，在 per-session runtime state。后端投影 `SessionMessage`（`apps/desktop/src/types/hermes.ts:544-582`）经 `toChatMessages` 转为渲染模型（`chat-messages.ts:922` 起：工具行折叠、`display_kind` 转 system 行、`@image:` 行提取回 attachmentRefs）。part 渲染细节见消息渲染器笔记。
...
- `_flush_messages_to_session_db_unlocked`（`run_agent.py:2010` 起）：收集本轮新消息 → 单事务 `append_messages_batch` → 成功才盖章；失败不盖章、清空扫描前缀，下轮全量重扫。`append_messages_batch` 支持按 `chunk_rows` 分块事务（`hermes_state.py:7781, 7818-7823`，分支复制用 500 行/块）。`_persist_session`（`run_agent.py:1900`）在每处退出路径（完成、错误、关闭）调用它。
- 工具执行增量落库：`_flush_session_db_after_tool_progress`（`agent/tool_executor.py:178`，在 `:800/:1504/:1645/:1677/:2269/:2340` 等工具批次点调用），应对工具杀死进程的场景。
- 崩溃痕迹：`tui_gateway/turn_marker.py` 写 `<home>/desktop/interrupted_turns.json`（`_MARKER_DIR/_MARKER_FILE` `:34-35`；`record_turn_start:97`，上限 32 条/24h/64KB，`:36-40`；`clear_turn_marker:124` 在任何“客户端已见到结果”的结局清）。`session.resume` 时按标记自动重跑，attempts 计数防崩溃循环。中途断连的另一层恢复是 `_fail_inflight_turn` 保留的可重放快照（`server.py:7343`，`session.resume` 的 `inflight` payload 承载，见对话请求笔记 §6）。

### 2.4 标题（两段式 + 带来源）

`agent/title_generator.py` 的命名现在是**两段式 + 带来源**：

- **即时标题（derived）**：会话首个 turn 的 prologue 内同步执行（`turn_context.py:188` `_maybe_title_session_at_turn_start`，经 `maybe_auto_title` `:234-255` 派发）——`apply_instant_title`（`title_generator.py:474`）调 `derive_title`（`:250`，无模型调用、不失败）从开场消息取首个有效行、折叠空白、词边界截断（`MAX_DERIVED_TITLE_CHARS=48`，`:64`），用户发出首条消息后毫秒级可见；斜杠命令等控制包装被剥离（`is_titleable_user_message`，`:236`）。随后 `auto_title_session`（`:508`）在后台线程用小模型升级（约束 JSON `{"title": ...}`）。
- **来源与覆盖（provenance）**：`set_auto_title`（`hermes_state.py:6688`）是“优先级检查 + 写入”的单事务 compare-and-swap（`_set_session_title`，`:6587-6668`），authority 序 `derived(0) < llm(1) < user(2)`（`:6479-6485`）；用户 `/title`（`session.title` RPC，`methods_session.py:1022`，`set_session_title` 记 `user` 来源）永不被自动标题覆盖，后台 `llm` 标题也不会覆盖用户标题；legacy NULL 行按 user 计，自动标题只填空（`:6499`）。唯一索引冲突时经 `get_next_title_in_lineage`（`:6969`）加 `#N`（derived 阶段 `dedupe=False` 让出冲突，交给后台重拾，`title_generator.py:421-471`）。
...
DB 行是惰性创建的：`AIAgent._ensure_db_session` 在首次 turn 时 `create_session` INSERT（`run_agent.py:628-667`，失败保活下轮重试）；TUI 侧 `_ensure_session_db_row`（`server.py:2727`）也在首次 prompt 时落行，废弃草稿不留行（`methods_session.py:113-118` 注释）。`SessionDB.create_session` → `_insert_session_row`（`hermes_state.py:3859, 3685`），ON CONFLICT 时从父行继承 cwd/git/profile。

### 3.2 resume / close / save

- `session.resume`（`methods_session.py:306` 起）：先 `resolve_resume_session_id`（`hermes_state.py:8566`）沿压缩链跳到活 tip（`methods_session.py:374`），`reopen_session` 清 `ended_at/end_reason`（`hermes_state.py:4791`）；`get_resume_conversations`（`:8854`）一次 SELECT 出两份投影——模型喂入用修复交替版、显示用原样版（`methods_session.py:545, 631`），`sanitize_replay_history` 去掉悬空 tool 尾巴（`:554, 642`）。resume 有**转录安全上限**：`sessions.max_resume_messages`（config-gate，默认 20000，0 关闭；`hermes_state.py:89-127` `MAX_SAFE_RESUME_MESSAGES`/`resolved_max_resume_messages`，`config_defaults.py:2836`），在 reopen 前计数整条压缩链（`methods_session.py:381-410`，超限错误 4130）。
- `session.close`（`methods_session.py:2748`）→ `_teardown_popped_session`（`server.py:905`）→ `_finalize_session`（`server.py:661`）：flush 未写消息（明确**不带** `conversation_history` 调 `_persist_session`，避免全部消息按身份视为已持久化而跳过，`:686-703`）、调 `on_session_end` hook（`:711-725`）、`end_session` 标记 `ended_at/end_reason`（`:737-761`，first-reason-wins，gateway 来源会话不归 TUI 所有不标记，`hermes_state.py:4773`），不删数据。
...
- `session.delete`（`methods_session.py:972`）：拒绝删除进程内活跃会话（错误 4023，`:1002-1003`）；`delete_session` 硬删除 messages + sessions 行（`hermes_state.py:9526-9583`），delegate 子代理级联删（`:9566`），分支/压缩子会话则 `parent_session_id → NULL` 孤立保留（`:9568-9572`）；连带清理磁盘转录文件（`:9580-9582`）。
...
- 保留语义：轮转与原地压缩都以 `active=0/compacted=1` 保留旧行，压缩归档行仍可搜索；`end_session` 只标记不删数据；硬删除无回收站。`/retry`、编辑/rewind 截断均改为**保留归档历史**的破坏性改写：`replace_messages` 带 `active_only=True, archive_dropped=True` 语义（`methods_prompt.py:264-286` 注释引 #70516/#80763/#82756），旧轮次不再从消息列表蒸发、但落盘为可搜索归档行。

## 4. 编辑、重试、续写、回退与分支语义

- **编辑 = 截断 + 重发**。无 `session.edit`/`message.edit` RPC（检查范围：`tui_gateway` 全部 `@method` 注册表，`session.*`/`message.*` 方法列表中无编辑项；桌面侧无对应调用）。编辑经 `prompt.submit` + `truncate_before_user_ordinal`（桌面 `rewind.ts:37` `truncateSubmitParams`）；服务端先 `db.replace_messages`（单事务 DELETE+重插，压缩已结束会话抛 `CompressionSessionClosedError`，`hermes_state.py:2023, 8241`）再改内存历史并 `history_version += 1`（`methods_prompt.py:175-303`）。桌面入口 `user-message.tsx:326-355`：点击进入编辑 composer，发送即“interrupt + rewind”（执行链在对话请求与上下文笔记 §7）。
- **删除**：仅会话级，无单消息删除（检查范围：方法注册表 + 桌面端消息操作代码，无 message-level delete RPC）。软删原语 `rewind_to_message`（`hermes_state.py:9084`，id≥target 全部 `active=0`）只服务 `/rewind` 命令（`session.undo` 是进程内历史截断 + `replace_messages`，见下）。
- **重试**：无 `session.retry`（检查范围：`@method` 注册表）；`truncate_after` 概念本仓库不存在（全树 grep 仅命中无关测试名）。三套实现共享“截断到最后一个真实 user turn 后重发”语义，真实 user turn 判定 = `is_user_originated_turn`（`agent/context_compressor.py:7368-7386`：`role=="user" and not display_kind` 且非压缩摘要/合成轮）；TUI `/retry`、slash.exec、桌面 reload/regenerate 与 CLI `retry_last()`（`cli.py:8714`）的执行细节见对话请求与上下文笔记 §7。
- **续写**：无 `/continue` 命令、无独立 RPC，就是普通 `prompt.submit` 文本 “continue”（busy 时也只入队）。空响应恢复会重放 `assistant("(empty)")`，持久化前由 `_drop_trailing_empty_response_scaffolding` 删除（`run_agent.py:1940`，`turn_finalizer.py:268` 调用）。
- **分支**：`session.branch`（`methods_session.py:2760` 起）：可选 `count` 截断复制历史 → 新 `session_key` + `model_config={"_branched_from": old_key}` + `parent_session_id=old_key`（`:2793-2813`）→ `append_messages_batch` 分块复制（chunk 500，保留原 timestamp，`:2817-2842`）→ 标题 `get_next_title_in_lineage` 生成 `"xxx #2"`（`:2788-2792`）；父会话保持 live；返回 `{session_id, stored_session_id, parent, messages}`。分支种子在首次 prompt 时经 `_persist_branch_seed` 落库（`server.py:2859`，`methods_prompt.py:326`）。桌面 fork：live 会话用 `session.branch`，无 live 源时 `session.create` + `parent_session_id` + messages 种子（`use-session-actions/index.ts:1178` `forkBranch`）。
- **版本切换**：无 checkpoint 版本 RPC。`session.history` 读含祖先的全量正序历史（`include_ancestors=True`，`methods_session.py:2442-2463`）。`session.steer`（`:3219`）与 `session.redirect`（`:3252`）是运行中干预，不新增持久化对象，执行语义在对话请求与上下文笔记 §7。`session.undo`（`methods_session.py:2466-2501`）拒绝 running 中调用（错误 4009），按 `is_user_originated_turn` 找最后一个真实 user turn 截断进程内历史并 `history_version += 1`。
...
- REST `GET /api/sessions`（`hermes_cli/web_routers/sessions.py:54-163`）：`limit`（默认 20、上限 100，`:58`）、`offset`、`min_messages`、`archived`、`source(s)` 过滤。
- 底层 `list_sessions_rich`（`hermes_state.py:7104`）：单查询 + 预览子查询（取首个 user 消息）；`include_pinned` 把被 LIMIT 挤出的置顶会话回填（`:7164-7170`）；最近活跃按压缩链递归 CTE 取 tip 的 `effective_last_active` 并在 SQL 层 LIMIT（`:7144-7150`）；压缩链 root 投影到活 tip（一条逻辑会话一条目，`:7136-7142`）；默认排除子代理与压缩续链会话（`_LISTABLE_CHILD_SQL` + `_delegate_from IS NULL`，`:7182-7198`，`hermes_state_common.py:103`）。
...
- REST `GET /api/sessions/{id}/messages`（`sessions.py:601-635`）：`limit` 上限 500（`:629`），`order=oldest` 从头翻页、缺省/`latest` 返回最新一页（`:627-628`）；先 `resolve_session_id` + `resolve_resume_session_id` 沿压缩链解析（`:618-621`）。
- `get_messages_as_conversation`（`hermes_state.py:8655`）：`ORDER BY id` 正序（不用 timestamp，防 WSL2/NTP 时钟回拨打乱 tool_call 邻接，注释在 `:8691-8698`）；`include_ancestors` 先走 lineage 链（`:8682-8683`）；`repair_alternation` 供 LIVE REPLAY（`:8671-8679`）。
...
- 索引矩阵：`messages_fts`（fts5 unicode61 基表，`hermes_state_common.py:417`）、`messages_fts_cjk`（bigram，`cjk_unicode61`，`hermes_state.py:1930`，绝大多数 CJK 查询走它）、`messages_fts_trigram`（≥3 CJK 字，`hermes_state_common.py:486-492`，通过排除 tool 行的视图 `messages_fts_trigram_src` `:481`）、LIKE 兜底（短/混合 CJK 或 role=tool）。tool 行不进 trigram 索引（注释明示约 90% 字节是机器噪声，`:471-479`）；路由路径分类 `_describe_search_path`（`hermes_state_search.py:1460-1482`：fts5 / fts_cjk / trigram / like_scan，`_search_messages_impl` 内按同类条件分派）。
- 命中定位：FTS 路径 `snippet(...,40)` 前后端标（`:1387/:1820`）；LIKE 路径 `instr` + 前后字符切片（`:1580`）；`context` 字段返回命中前后各 1 条（`:1625-1685`）；结果默认剥掉 `content` 只留 snippet（`:1687`）。FTS5 特殊字符剥离（`:1237-1239`：非 CJK 查询剥 `%` 等）。
...
- `SCHEMA_VERSION = 25`（`hermes_state_common.py:167`）；v25 起系统提示词去重到 `system_prompts` 表（`system_prompt_hash`，`:202-205, 221`）。FTS 存储布局版本独立于 SCHEMA_VERSION（`FTS_STORAGE_VERSION`，`:170-178`），legacy DB 在用户执行 `hermes sessions optimize-storage` 前停留在旧布局。
...
- **子代理**：`delegate_tool.py:1305` 起 `_build_child_agent` 构造 `AIAgent(platform="subagent", session_db=父会话共享句柄, parent_session_id, skip_memory, quiet_mode)`；子代理有独立 session_id（`agent_init.py:1512-1519`），首轮惰性落行时写 `_delegate_from` 标记（`delegate_tool.py:1679`）。列表隐藏是双层：`_LISTABLE_CHILD_SQL` 排除无 `_branched_from` 的子会话（`hermes_state_common.py:103`）+ `_delegate_from IS NULL` 显式排除（`hermes_state.py:7198`），父删除后孤儿化仍隐藏。删除时沿标记递归级联（`_collect_delegate_child_ids`，`hermes_state.py:287`），未标记子会话走“孤儿化不删”契约防环。
- **工具调用的消息表示**：assistant 行带 `tool_calls`；结果经 `make_tool_result_message` 落 `{"role":"tool", tool_call_id, ...}` 行，高风险工具内容包 `<untrusted_tool_result>`（`agent/tool_dispatch_helpers.py`）；中断/取消工具写 `effect_disposition="none"` 占位（`agent/tool_executor.py`）。UI 投影 `_history_to_messages` 中 tool 行渲染为 `{role:"tool", name, context}`（`server.py:7136`），assistant 的 tool_calls 不单独成行。
