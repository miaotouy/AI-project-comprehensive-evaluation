...
- **惰性落行**：DB 行首次真实 turn 才创建，空草稿不留行（`run_agent.py:621-666`）。
...
压缩轮转的数据语义：`conversation_compression.py:3265-3329`——生成新 session_id → `publish_compression_child` 单事务插入子行（继承 cwd/git/profile/gateway origin）并写压缩后消息、父行 `end_reason='compression'`（`hermes_state.py:3488-3589`）→ `agent.session_id` 切换 → flush 基线重置；失败回滚父会话（:3331-3375）。原地压缩 `archive_and_compact` 不轮转 ID，软归档 `active=0, compacted=1`（`hermes_state.py:6938-7010`），同一会话 ID 贯穿一生。压缩的触发与预算语义在对话请求与上下文笔记 §3。
...
- 崩溃恢复：SQLite 只在 turn 结束写，`interrupted_turns.json` 是唯一中途痕迹；`session.resume` 时按标记自动重放（`server.py:7241-7322`），attempts 计数防崩溃循环。
- 压缩轮转恢复：resume 沿压缩链跳到活 tip，`get_resume_conversations` 提供模型喂入与显示两份投影（3.2）；`_lineage_root_id` 提供跨轮转的稳定身份（`hermes_state.py:6150`），桌面端 pin、路由匹配都基于它（`session.ts:243-252`）。
- 保留语义：轮转与原地压缩都以 `active=0/compacted=1` 保留旧行，压缩归档行仍可搜索；`end_session` 只标记不删数据；硬删除无回收站。
...
- **版本切换**：无 checkpoint 版本 RPC。`session.history` 读含祖先的全量正序历史（`include_ancestors=True`，`methods_session.py:2357-2378`）。`session.steer` 与 `session.redirect` 是运行中干预，不新增持久化对象，执行语义在对话请求与上下文笔记 §7。
...
- 桌面侧栏走 `/api/profiles/sessions/sidebar` 分 profile 分片（`hermes.ts:541-585`），`SIDEBAR_SESSIONS_PAGE_SIZE` + `loadMoreSessions` 翻页（`layout.ts`、`use-session-list-actions.ts:242-268`）；侧栏界面工作流与虚拟列表呈现见 Chat UI 笔记 §2。
...
- **压缩轮转的数据一致性**：轮转后 flush 基线重置；`_sync_session_key_after_compress` 重锚 session_key（`server.py:9796-9798`，实现 :4750-4837：会话配额租约迁移、yolo 状态迁移）。
...
- **附件（数据侧）**：无附件表，图片经 `@image:` 指令或 vision 预分析转文本，落库时多模态图片部分被压成 `[screenshot]`（`run_agent.py:2206-2215`）；`[[Image N]]` 指引本次未找到（全仓库 grep 零命中）。桌面拖放/上传管线属于 Chat UI 笔记 §3，提交注入点属于对话请求与上下文笔记 §9。
...
- `hermes_state.py`：`create_session`（:3098）、`append_messages_batch`（:6454）、`replace_messages`（:6866）、`rewind_to_message`（:7610）、`set_session_archived`（:5465）、`delete_session`（:8052）、`get_messages_as_conversation`（:7265）、`list_sessions_rich`（:5790）、`get_resume_conversations`（:7464）、`publish_compression_child`（:3488）/`archive_and_compact`（:6938）、`_lineage_root_id`（:6150）。
- 运行与持久化：`run_agent.py`——`_persist_session`（:1889）、`_flush_messages_to_session_db_unlocked`（:1999）、`_ensure_db_session`（:621）。
- session 方法：`tui_gateway/methods_session.py`——create（:14）、list（:162）、resume（:306）、delete（:887）、save（:2588）、close（:2660）、branch（:2672）、history（:2357）、title（:937）。
- 崩溃标记：`tui_gateway/turn_marker.py`；标题：`agent/title_generator.py`；搜索：`hermes_state_search.py` `search_messages`（:1308）、`search_sessions_by_id`（:2026）。
- 子代理：`tools/delegate_tool.py` `_build_child_agent`（:1512）。
- 桌面端数据投影：`apps/desktop/src/types/hermes.ts`（`SessionMessage` :538）、`lib/chat-messages.ts`（`ChatMessage` :13、`toChatMessages` :912）、`store/session.ts`（`mergeSessionPage` :334、`sessionPinId` :243）、`use-session-list-actions.ts`（:49-67、:142-181）。
