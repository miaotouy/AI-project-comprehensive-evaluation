...
> 汇总更新日期：2026-08-18
>
> 依据：Agent 工具、Agent 角色、Chat、Chat UI、LLM 渠道管理、仓库分布、会话与消息管理、外部执行体与应用协作、对话导出与分享、对话请求与上下文、应用界面基础设施、消息渲染器、独特功能、生成式输出与运行时共 14 个类目的 DeepSeek-Harness 调查笔记（完整清单见文末来源笔记索引），全部基于同一代码快照 `47f943859bef60e4160492346772ded9b24f765a`（分支 `master`）
>
...
口径说明：本汇总所有"主链确认 / 静态源码确认"均基于对当前代码快照的源码贯通。DeepSeek-Harness 是完整本地主链（agent 循环、会话日志、工具执行、Web UI 均在本仓库内），在编译型桌面应用或完整本地主链中视为完成交付态；"未运行验证"仅指未进行黑盒运行、UI 或端到端操作，不否定代码完备性，后续黑盒验证属于补充确认而非缺陷修复。
