...
工作台的独特之处在于结果有独立于消息的 inlay 身份，但它不是完整的媒体工程：资产没有创建来源、提示词、模型、版本、父子关系或反向引用索引。图像生成设置是全局/角色配置，历史参数不随 inlay 保存；Playground 的手动生成当前只显示返回 data URI，静态路径未见其自动写入 inlay，因此可继续复用的正式主链是聊天收口后的 inlay 写入。
