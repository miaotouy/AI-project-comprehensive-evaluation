...
**参数清洁**：表单字段由 `imageGenerationToFields` 从 registry support 动态生成（`PaintingComposer.tsx:134-138`）；提交时 `canonicalGenerate` 用 `buildParamsSchema` 校验/强制（seed 字符串→数字、enum/range 边界、customSize 合成 `WxH`、空值丢弃，`canonicalGenerate.ts:94-122`）；main 侧 `splitParamValues` 再把 canonical bag 分成 AI SDK 结构化字段（n/size/seed/aspectRatio）与 vendor 剩余袋（cfg 等），"字节一致 wire"不变式（`imageOptions.ts:26-40`）。编辑模式（非 generate）强制要求输入图（`EDIT_IMAGE_REQUIRED`，`canonicalGenerate.ts:78-83`）；输入图以 `data:` URL 随 IPC 发送（`canonicalGenerate.ts:124-136`），main 按 `cleanupPolicy: 'delete_when_unreferenced'` 持久化（`generatePainting.ts:54`）。

**执行位置**：全部在 main。同步路径 `aiCoreGenerateImage` 用 AI SDK ImageModel 调用，vendor wire 映射走 WireProfile 引擎（`AiService.ts:765-817`，含 `experimental_download` 兜底下载 URL 结果）；异步路径 job handler 拥有 submit/poll/download/persist 循环（`imageGenerationJobHandler.ts:100-137`），secrets 不落 job（apiKey 每次执行重读，`imageGenerationJobHandler.ts:27-30`）。渲染侧无任何直连模型代码。
