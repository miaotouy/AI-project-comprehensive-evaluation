...
**设计取舍**：
- 媒体能力全部走 Agent 工具协议而非用户 UI：创作入口、参数清洁、结果展示都依赖模型遵循 manifest 契约，产物 URL 即持久化与复用介质，与 AIO Hub 桌面工作站、Chatbox 图像工作台的产品形态不同。
...
- 音频合成把 AI 生成的 DSP 代码交给独立 Node 子进程执行，给予完整 DSP 能力但以验证码与采样上限约束，产物由主进程二次校验。
