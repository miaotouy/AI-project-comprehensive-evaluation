...
- 横向比较对象建议为同类 harness：Pi（扩展系统/分支会话）、OpenCode（多表面/子代理/ACP）、Hermes Agent（skill 闭环）、Jan（编排端点）；"agent 修改自身运行时"保留为稀有能力卡。
