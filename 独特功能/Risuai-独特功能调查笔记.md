...
Risuai 与 SillyTavern 同属角色扮演聊天品类，但产品辨识度集中在两条主线上：**把“模型输出”与“角色媒体表现”接成闭环的指令标签链**，以及**本地优先的完整工具链**；角色卡与世界书属通用类目基线。本次确认六项独特能力主链（全部静态证据），另有一项 README 外候选（Visual Novel 模式）在本快照未找到实现：
...
**用户目标**：让角色立绘随剧情情绪自动切换，并把“情绪表达”做成模型驱动、可随角色卡分享的媒体工作流。
...
**独特性判断**：这是“模型指令标签 → 媒体执行”的完整闭环（与 Additional Assets 的 `{{img::}}`/`{{bg::}}` 标签同一解析器，`parser.svelte.ts:408`），且带独立分类请求兜底；SillyTavern 的表达式系统只到 `入口确认` 且依赖 Extras API/classify 外部模块，Risuai 路径完全本地闭环。`req.special.emotion` 字段（把分类结果回填立绘的第三条路径）在快照中未找到写方，属于遗留类型定义，实际不活跃。
...
**用户目标**：把“文件”作为对话工作对象批量送入模型，并按文件类型定制处理流程（逐条翻译、语义剪裁、媒体入消息）。
...
- **辅助贡献候选**：翻译系统（结构保留翻译，参照 Cherry Studio 先例）、Multisend（文件工作对象，含 .po 流水线）、WebRTC 多用户房间（稀有能力卡）、Risu Hub 客户端接入（外部依赖标注）。
