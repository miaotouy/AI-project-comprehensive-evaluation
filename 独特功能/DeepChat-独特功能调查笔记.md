...
**入口与触发者**：设置中的 Remote 页面（`src/renderer/settings/components/RemoteSettings.vue`），五个渠道分 tab：Telegram、Feishu/Lark、QQBot、Discord、WeChat iLink。触发者始终是 IM 平台的外部消息/回调事件。

**事实对象**：远程端点绑定对象以渠道私有聊天或群标识作为 endpoint，绑定到一个 DeepChat session；PairCode 授权有效期为 10 分钟，失败上限为 5（`src/main/remote/types.ts:33-37`）。
...
**主动性**：远程侧可用 `/stop` 中止生成、用 `/pending` 查看并答复挂起交互、用 `/open` 把会话带到桌面。定时任务（CronJobs）支持 remote delivery 目标（`CronJobsSettings.vue` 的 `remoteDelivery` 配置），计划任务结果可投递到已绑定 IM 端点，远端不是只读终端。
...
**事实对象**：Tape 是一个结构化记录系统，核心对象是 Tape entry（用户消息、工具调用、provider attempt 等事实）与 view manifest（一次请求的“视图快照”）。模块采用完整 DDD 分层，领域层、应用层和 SQLite 基础设施层分别位于 `src/main/tape/domain/`、`application/` 和 `infrastructure/sqlite/`，包含记录、事实、回放、视图、召回和生命周期等服务。

**执行日志与契约谱系**（静态确认）：

- **严格执行日志**（`tape/domain/executionJournal.ts` + `application/executionJournalService.ts`）：以 `execution/run_started`、`execution/dispatch_committed`、`execution/tool_outcome`、`execution/run_terminal` 四类事件记录 run 边界与工具分派，带 provenance key、canonical JSON 比对和协议版本；启动时按日志行分类恢复，crash 场景可识别未收口 run。
- **执行契约**（`tape/domain/executionContract.ts`，上限：契约 64 KiB、工具 256、subagent 深度 1）：每次请求按工具、工作区和深度冻结 ceilings，工具服务在分派前校验；绑定 id 写入 assistant block 的 `executionContractBinding`（`src/main/tool/index.ts:329`、`:388`、`:475`；字段见 `src/shared/chat.d.ts:199`）。工具执行结果与 `providerReplayJson`（见能力卡 4）都由日志约束。
- **任务契约与评估**（领域层与应用层的任务契约、评估服务）：delegation 子任务在冻结时继承父契约（`contract/task_frozen`），完成后按 `contract/evaluated` 事件评估；补偿或压缩造成的顺序位移也会写入 order 类型的消息替换事实（`transcript.ts:258-298`）。
...
**用户目标**：让模型自主联网搜索，并高亮信息来源。README 声明三块：内置搜索 API（BoSearch/Brave）、模拟浏览器读取搜索引擎，以及“配置搜索助手模型连接任意搜索源”（`README.md:115-118`）。
...
**声明不符项**：README 中“配置一个搜索助手模型连接各种搜索源（内网、无 API 引擎、垂直搜索引擎）”对应 i18n 键 `searchEngineName/searchEngineUrl`（`src/types/i18n.d.ts:2182-2186`），但 `src/` 内的 .ts/.tsx/.vue 仍无消费方。因此判定为“遗留声明/未接线配置”，当前可执行路径以 Bocha/Brave/DeepResearch、YoBrowser 和 DeepSeek 原生搜索为准。
...
**边界**：下载进度事件、并发 pull、Ollama 服务未运行时的 UI 行为未运行验证；模型拉取入口返回 boolean，但进度另走事件通道。
...
**边界**：URL 协议注册的平台差异（Windows/macOS/Linux）未运行验证；协议参数经 URL 传递时的凭据暴露面由调用方承担。

### 能力卡 7：CLI 本地控制平面（终端驾驶面）

**用户目标**：在终端里用 deepchat CLI 直接发起 agent run、管理 MCP/Skill/Provider/设置、做 OCR/转写/媒体生成，桌面应用作为后台宿主执行——与 IM 远程控制（能力卡 1）同属“多表面连续性”方向的终端面。

**入口与触发者**：CLI 可执行文件由构建脚本随主应用产出；首次使用经 launcher 安装或授权，设置页提供管理入口（`scripts/build-cli.mjs`、`launcherService.ts`）。触发者是终端用户或脚本。

**事实对象**：本地 HTTP 控制面由 CLI 服务提供，监听 localhost 并通过带 scope 的 token 鉴权；RPC 路径包括 `/rpc`、`/stream`、`/upload`。控制协议定义版本、方法表、事件信封和 JSON 响应、流记录、上传头等边界，命令再按 12 组路由面装配，覆盖 80+ 个 route（`cli/server.ts`、`agentTokenAuthority.ts`、`src/shared/contracts/localControl.ts`、`src/shared/contracts/cliCommands.ts`、`src/main/cli/routes.ts`）。
...
**持续性**：launcher 安装可逆，卸载或数据重置仍保留入口；detached run 有独立生命周期与终端状态等待（#2136）；descriptor 文件供 CLI 发现宿主布局（`LOCAL_CONTROL_DESCRIPTOR_FILENAME`）。

**主动性与取消**：CLI 可发起 detached runs（后台持续执行）；fix(cli) 系列收紧了终端状态等待、JSON 安全输出与“只返回最终答案”。

**安全与资源边界**：所有命令经 token scope 授权；变更类命令走变更守卫和审批 broker；上传字节数、审批文本过滤、MCP 状态持久化等都有显式上限与失败归一化（fix(cli) 系列提交）；agent 命令访问受 `agentCommandAccess.ts` 约束，如阻止 CLI 提权管理自身。

**外部依赖与执行域**：全部在本机；CLI 只是主应用进程的远程接口，无云端组件。

**独特性判断**：与 IM 远程控制互补：IM 面由外部消息平台驱动，CLI 面由本机终端或脚本驱动，两者共享同一会话事实源与审批 broker。README 未把 CLI 列为旗舰，但代码已形成完整主链，使用与验证指南见 `docs/guides/cli.md`。标签：多表面连续性。
...
- **会话内/跨会话搜索**（自有数据搜索）：归并会话与消息管理笔记 §5 与 Chat UI 笔记 §2，已确认搜索入口、FTS5 和会话搜索服务。
...
- Feishu 渠道的安装与鉴权依赖飞书开放平台应用；Weixin iLink 依赖企业微信接口；Telegram/QQBot/Discord 依赖各自 bot 平台——均为外部服务依赖，主链执行需真实凭证（暂缓运行验证）。
