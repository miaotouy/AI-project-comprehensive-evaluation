...
此前"覆盖闭环、无独立独特能力卡"的结论部分成立、部分不成立。归并结论正确的部分是：严格 Artifact（`HtmlArtifact.tsx` 仅是围栏 iframe 预览，无独立对象与生命周期）、本地/云统一路由（按 `model_id` 的静态三路解析，无动态调度）、Hub 模型下载、RAG 附件检索与 web_search 工具。被遗漏的是以下达到 `主链确认`（静态证据）的新候选：
