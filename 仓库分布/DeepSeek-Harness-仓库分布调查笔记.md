...
>
> 调查范围：覆盖模块、语言、文档、测试、跨平台与工程配套分布；排除运行时行为、性能与构建产物；仓库为 32 条提交的浅克隆，快照之外的历史未纳入
>
...
| 文档文件数/集中位置 | 2,421 / 170,252 行；`.agents/notes` 1,386、`docs/` 215、`packages/` 593 |
| 测试文件数/集中位置 | 1,772 / 307,244 行；`packages/` 991、`apps/` 237、`examples/` 489（多为回放夹具） |
...
| --- | ---: | ---: |
| 跟踪文件 | 7,412 | 7,338 |
| 源码文件 / 行 | 2,756 / 590,509 | 2,682 / 583,943 |
| TypeScript 行 | 564,122（95.5%） | 约 557,600（95.5%） |
| 文档文件 / 行 | 2,421 / 170,252 | 2,409 / 169,434 |
| 测试文件 | 1,772 | 1,772（vendor 无测试） |
...
测试资产 1,772 个文件 / 307,244 行测试源码行（占全仓源码行 52%）。按类型与位置：
...
边界与限制（已确认）：仓库是 32 条提交的浅克隆，HEAD 为发布合并后的 rc.5 状态；`vendor/` 的同步目标在清单中声明为外部 fork（deepseek-harness/cordis 等），本次未核对这些上游 URL 与清单一致性之外的演进；所有统计均为静态口径，行数不等于功能量。
