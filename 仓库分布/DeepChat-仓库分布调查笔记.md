...
>
> 调查范围：模块、语言、文档、测试、跨平台、工程配套（CI/脚本/国际化/资源/插件）与结构特征；未运行构建与测试；统计底稿以提交范围 `dc4177c2..e142b2a` 后的 HEAD 为准
>
...
本笔记采用《仓库分布调查指南》的统一口径，快照为当前 HEAD commit `e142b2a2` 的 Git 跟踪文件：

- **快照边界**：只统计 `git ls-files` 跟踪的 4,379 个文件；不统计 `.git`、未跟踪文件、`node_modules`、构建输出（`out/`、`build/`）与运行时数据。
