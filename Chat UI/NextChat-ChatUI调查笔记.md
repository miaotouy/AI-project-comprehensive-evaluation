...
- 输入区支持文本、图片附件、以冒号开头的聊天命令和以斜杠开头的 prompt 提示；草稿是组件局部状态，未完成输入按会话存 localStorage，空输入时按上方向键可回填上一次输入。
...
- 组件局部 state（不持久化，切换会话重挂载即丢失）：输入文本与附件、上传/加载/渲染索引等运行标记、滚动相关标记、提示词提示、聊天侧面板与语音状态共 10 项，声明见 `app/components/chat.tsx:998-1035, 1287-1288, 1386-1437, 1600-1680`：
