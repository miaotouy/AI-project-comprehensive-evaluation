...
- **聊天主链**：`Chat.vue` 工作台（会话列表 + 消息区 + Composer）→ `useMessages` 以 SSE（POST `/api/v1/chat`）或 WebSocket（`/api/v1/unified-chat/ws`）双通道提交并消费流式事件 → 事件进入 webchat 队列 → 与外部 IM 共享同一套事件总线 + 流水线（执行语义在对话请求与上下文笔记）；`scheduler.execute` 结束时 `send(None)` 触发 `end` 事件刷新 UI（scheduler.py:94-95）。
...
- **入口与路由**：`/chat`（views/ChatPage.vue，15 行壳组件）与 `/chatbox`（views/ChatBoxPage.vue，`chatboxMode=true` 全屏）都渲染 `components/chat/Chat.vue`（router/MainRoutes.ts:244-250、ChatBoxRoutes.ts:2-13）。
- **工作台拓扑**（Chat.vue）：`v-navigation-drawer` 侧栏含 ProjectList（会话列表 :104）；`main` 区（:328-332 挂 `dragEvents` 拖放热区）分三种形态：Provider 工作台（:341-346）、项目视图 ProjectView + Composer（:348-389）、常规会话视图（消息区 ChatMessageList :391-439 + Composer :441-475）。消息区与 Composer 分离，无独立辅助面板；推理/引用/线程以侧边抽屉呈现（ReasoningSidebar、RefsSidebar、ThreadPanel :535-540）。
...
- **会话列表**：ProjectList 展示 `chatApi.listSessions` 返回的 PlatformSession 行（useSessions.ts:34-48），支持新建（:50-80，创建后绑定所选配置 profile 并跳转路由）、删除（:82-91）、批量删除（:121-158，按 failed_items 判定当前会话是否被删）、重命名（对话框 :160-184）。会话可归入项目（ChatUIProject）分组，项目展开时懒加载会话（Chat.vue:1161-1188），项目内会话可独立编辑标题/删除（:1271-1285）。
...
- **现场恢复**：`loadSessionMessages`（useMessages.ts:257-283）加载历史 + threads + `active_runs`；`restoreNextActiveRun`（:285-313）把运行中 run 的快照插回消息列表（占位 `isLoading`）并 `startResumeStream`（:679-764，5 次指数退避重连，收 `end` 才停）。滚动锚定：`shouldStickToBottom` 距底 80px 内吸底（Chat.vue:1713-1729），流式更新时吸底滚动（:808-812）。
...
- **草稿**：`draft` 是组件内存 ref（Chat.vue:711、StandaloneChat.vue:258、ThreadPanel.vue:94），**不持久化**（localStorage 仅存 transportMode/selectedProvider/selectedModel/projectExpandedIds，静态检查）；切换会话清空 replyTarget 但保留 draft 值本身（draft 未按会话分组，属全局草稿——静态推断）。
- **附件**：图片/音频/文件经 `useMediaHandling` 上传为附件并 staging 预览（Chat.vue:667-680）；拖放热区为整个聊天区域（useDragUpload.ts:7-47，拖入显示全区遮罩 Chat.vue:333-340），drop 后走 `handleFilesSelected` 同一上传链路（Chat.vue:682-685、:1676-1685）；粘贴图片直接上传（`@paste-image` :384）；录音按钮 + Ctrl+B 长按录音（ChatInput.vue:723-733，useRecording）。
- **回复引用**：消息操作触发 `replyTarget`，发送时作为 `reply` part 携带（Chat.vue:1403-1411）。

## 4. Agent、模型、工具与发送前配置

- **发送前配置**：Composer 提供 ProviderModelMenu（ProviderModelMenu.vue，选 provider/模型后写入 `selected_provider`/`selected_model`，经 `localStorage` 记忆 :217-229），随每次发送传给后端（Chat.vue:1384-1394）；流式开关 `enableStreaming`（:743、:1687-1689）；配置 profile 选择器 ConfigSelector（ChatInput.vue:144，数据来自 `/api/v1/chat/configs`，chat.py:247-252）。
...
- **发送**：`sendCurrentMessage`（Chat.vue:1342-1401）——无会话时先 `newSession()`，项目内发送则关联项目并刷新列表（:1350-1366）；本地先建 user/bot 占位记录（createLocalExchange，useMessages.ts:315-369），bot 占位 `isLoading`；随后 `sendMessageStream`（:371-409）按 transportMode 走 SSE（`startSseStream` :615-677，AbortController 管理）或 WebSocket（`startWebSocketStream` :766-802，每会话一个 WS 连接）。
- **流式反馈**：`processStreamPayload`（useMessages.ts:990-1160）逐事件更新 bot 记录——`run_started` 定位 run、`run_snapshot` 恢复快照、`user_message_saved`/`message_saved` 把本地占位替换为持久化 ID（:1059-1078）、`agent_stats` 写入统计、`follow_up_captured` 把用户插话记录移到运行中 bot 之前（:1004-1032）、plain/reasoning/tool_call 增量合并（:1113-1129）、错误以文本追加到消息底部（:1084-1088）。
...
- **编辑**：消息操作栏打开编辑（`openMessageEdit`，Chat.vue:1482-1486），保存调 `chatApi.updateMessage`（useMessages.ts:411-436）；成功且 `needs_regenerate` 时自动续写（continueEditedMessage :448-484）或重生成下一条 bot（Chat.vue:1505-1525）。仅最新用户消息可编辑、运行时禁用（后端校验见会话与消息管理笔记 §4）。
...
- **多会话**：`messagesBySession` 按会话分桶缓存在内存（useMessages.ts:143-144），切换会话即切换桶（`activeMessages` computed :154-158），已加载会话不重复拉取（`loadedSessions` :1334-1336）；`isSessionRunning` 全局判断任一会话是否在跑，当前会话的 bot 记录带 `is-streaming` 标记（Chat.vue:417-419）。
