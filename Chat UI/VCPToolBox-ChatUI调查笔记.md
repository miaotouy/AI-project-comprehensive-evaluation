...
- `server.js` 暴露的对话相关端点全部是纯 API（`/v1/chat/completions` 1217、`/v1/chatvcp/completions` 1231、`/v1/human/tool` 1250、protocolBridge 1247），没有任何配套 HTML 聊天页面；
...
AdminPanel-Vue 是 Vue 3 + Pinia + vue-router 的独立 SPA（`AdminPanel-Vue/package.json`：`vue ^3.5.31`、`pinia ^3.0.4`、`vue-router ^5.0.4`），构建产物在 `AdminPanel-Vue/dist`。
...
- 文档不一致记录：`docs/FRONTEND_COMPONENTS.md`（生成于 2026-02-13，`docs/FRONTEND_COMPONENTS.md:3`）描述的 AdminPanel 是"原生 JS/CSS + EasyMDE 的内嵌静态前端，不是独立 SPA 应用"（`docs/FRONTEND_COMPONENTS.md:26`、`:60`），与当前 AdminPanel-Vue 实现不一致；以当前代码为准。

## 容易被误判为聊天 UI 的地方（逐一拆解）

- **`FinalContextViewer.vue`**（`/final-context-viewer`，`manifest.ts:144-152`）：展示"最后一次发给上游模型前的最终请求体"，页面文案自述"**不包含 AI 最终输出**"（`src/views/FinalContextViewer.vue:26`）。数据来自内存中最多 5 组滑窗快照（`modules/finalContextStore.js:21` `MAX_SNAPSHOTS = 5`；前端 `maxSnapshots = ref(5)`，`FinalContextViewer.vue:511`），页面没有发送消息的输入框，空态文案是"尚未捕获任何最终上下文。请先发起一次聊天请求。"（`FinalContextViewer.vue:504`）——它是"事后调试镜像"。另带"池月1号算法验证"分析弹窗（`FinalContextViewer.vue:258-266` 模板、495-501 导入本地分析工具），对选中 AI 块之前的上文做词项证据分布统计，属本地可观测性工具（弹窗自述"外部可观测代理，不等同模型内部注意力"，266 行）；其算法行为未运行验证。
...
- **Nova 看板娘气泡**（`src/components/dashboard/VcpAnimation.vue`）：点击 Dashboard 侧边 Nova 头像弹出带头像 + 文字的气泡，台词来自硬编码静态数组 `NOVA_LINES`（159-191 行，约 30 条），`rerollNovaBubble()` 随机选一条本地文案（287-293 行），5 秒后自动关闭（`NOVA_BUBBLE_AUTO_CLOSE_MS = 5000`，150 行）。修正一处旧结论：气泡表情图并非纯本地——`ensureNovaEmojiPoolLoaded()` 会经 `emojisApi.getGallery` 从管理 API 拉取"Nova表情包"（251-285 行），失败时回退本地默认图；台词文本仍无网络请求。整体是彩蛋/氛围装饰，与聊天消息流无关。
- **沉浸观星模式**（`src/components/immersive/ImmersiveCelestialPanel.vue`，挂载于 `src/layouts/MainLayout.vue:12`）：连续点击 Logo 5 次触发（`EASTER_EGG_CLICKS = 5`，`VcpAnimation.vue:335`，2 秒窗口 336 行）。面板打开时经 `tarotDivinationApi.getCelestialSnapshot` 拉取真实星历快照（`ImmersiveCelestialPanel.vue:244-265`）并展示塔罗占卜卡片；属于装饰性功能，不承载聊天。
- **`aiChat` 后台 AI 代理**（`adminServer.js:320` 模块注册）：`routes/admin/aiChat.js:119-142` 提供 `/admin_api/ai/chat`、`/ai/chatvcp`、`/ai/models`，是把请求 JSON 代理到主服务 `/v1/chat/completions` 的纯 API（带超时），**避免前端暴露 Key**；`AdminPanel-Vue/src` 中未找到任何调用它的前端页面（全目录搜索 `aiChat` 无命中），即当前没有消费该代理的聊天 UI。
...
**(a) `OpenWebUI VCP Tool Call Display Enhancer.user.js`（v3.9.8，`:3`）**：用 `MutationObserver` 监视 CodeMirror 渲染出的 `.language-VCPToolCall` / `.language-DailyNote` 节点（`mount()` 333 行起、`MutationObserver` 379 行），按 `<<<[TOOL_REQUEST]>>> ... <<<[END_TOOL_REQUEST]>>>` 标记切分解析（`renderToolCall()` 217-243 行、`renderDailyNote()` 245-305 行），动态构建 `<div class="vcp-tool-card">` 表格卡片替换原始文本显示；另有兜底引擎扫描 `document.body` 文本节点（`processTarget()` 550-595 行、`initFallback()` 597 行起）。这一切发生在浏览器 DOM 层，没有额外网络请求。工具调用结果以纯文本出现在聊天消息里，是因为 VCP 工具调用走"纯文本标记协议"（`README.md:170`），且 `vcpInfoHandler.js:96-104` 把插件返回结果格式化成 `[[VCP调用结果信息汇总:...]]` 文本块、以 SSE chunk 形式写回同一条聊天流（`streamVcpInfo()`，`vcpInfoHandler.js:117-153`）。
...
**(c) `VCP_DailyNote_SidePanel.user.js`（v0.2.1，`:4`）**：在宿主聊天页面右侧挂独立侧边栏面板，展示 VCPToolBox 后端的"日记本"（DailyNote）管理界面。机制：`GM_xmlhttpRequest` 下载 `/AdminPanel/DailyNotePanel/` 三份静态资源（`download()` 101-103 行）→ 拼装成完整 HTML（131-179 行）通过 `<iframe srcdoc="...">` 注入（`initUI()` 197 行起、iframe 创建 257-261 行）→ 劫持 `window.fetch` 经 `__VCP_FETCH_PROXY__` + `GM_xmlhttpRequest` 附带硬编码的 `AUTH_USER`/`AUTH_PASS` 访问 `/AdminPanel/dailynote_api/*`（`__VCP_FETCH_PROXY__` 定义 63-97 行、凭据 42-43 行、fetch 劫持 156-164 行；后端路由：`Plugin/DailyNotePanel/index.js:191` 静态面板、`:219` 日记 API）。这是三者中唯一真正发起跨域网络请求的，但被调用的后端是 AdminPanel 生态下的笔记管理插件，与聊天消息流无关。
