...
独立于 `systemPrompt` 字段，还存在**三模式 PromptManager**（`Promptmodules/prompt-manager.js:6`，`currentMode: 'original'|'modular'|'preset'`），存储字段为 `promptMode` + `advancedSystemPrompt`/`presetSystemPrompt`：
