...
>
> 调查方式：基于当前 HEAD 的静态源码核对与旧笔记刷新；只读核对 agentConfigManager、agentHandlers、chatManager、settingsManager，未修改被调查仓库源码
>
...
- **模型参数可留空**：设置页折叠参数段的 temperature/contextTokenLimit/maxOutputTokens/top_p/top_k 输入留空时存为 `null`（`modules/settingsManager.js:181-185` 回填空值、`:276-282` 的 `parseOptionalNumberInput`、`:323-327` 保存时解析），发送前由 `chatHandlers.js` 的 `omitUnsetOptionalModelParams` 从请求体删除（`modules/ipc/chatHandlers.js:94-118`、`:1064`）。折叠段落未设置时显示"未设置"。
- **重新回复时附件文本重新提取**：`handleRegenerateResponse`（`modules/renderer/messageContextMenu.js:640`）在历史缓存了旧版固定 UTF-8 解码结果时，通过 `electronAPI.getTextContent` 从原始附件重新提取文本；`modules/fileManager.js:176-224` 的 `decodeTextBuffer` 新增 UTF-8/UTF-16 BOM、UTF-16 零字节启发式和 GB18030 兜底检测。
- 其余配置模型（目录结构、字段集、`{{AgentName}}` 宏、话题、Tavern Rules、头像、三模式 PromptManager）经核对未变，本刷新仅行号与上述两点变化。
...
| `temperature` | `number` | 温度（默认 0.7）；当前 HEAD 支持存 `null` 表示"未设置"，发送时从请求体省略（见刷新要点） |
...
- **preset**：从目录单选一个 `.md`/`.txt` 预设整段替换（`Promptmodules/preset-prompt-module.js:269` `loadSelectedPreset`、`:344` `getPrompt`；预设列表由 `modules/ipc/promptHandlers.js:65-124` 的 `load-preset-prompts` 扫描目录提供，默认目录 `AppData/systemPromptPresets`）。

### 3.4 发送时的配置引用与历史快照语义

- **发送用缓存引用**：`chatManager.js:1081` 发送时取 `currentSelectedItem.config`，不是每次读磁盘；刷新点只有三处——设置页保存后回写（`modules/settingsManager.js:401-421`，保存成功后重新拉取配置写回 `currentSelectedItem.config`）、加载话题列表时 `updateCurrentItemConfig`（`modules/topicListManager.js:431`）、历史 ≥4 条时 `attemptTopicSummarizationIfNeeded` 强制重读（chatManager.js:896-947，`:911-912` 附近写回配置）。
- **重新生成总是重读最新配置**：`modules/renderer/messageContextMenu.js:687` 的 `getAgentConfig` 重新读取 Agent 配置，systemPrompt/模型/参数在 :888-969 组装（:961-968 为 model/temperature/max_tokens/top_p/top_k；当前 HEAD 新增 `top_p/top_k` 处理与附件文本重新提取，`handleRegenerateResponse` 移至 :640）。
