...
`src/config/agent-presets/index.ts` 只登记预设元数据：`id`、名称、描述、图标、分类、标签和 `configUrl`。完整配置按 ID 放在 `public/agent-presets/{id}/`，可附带 `icon.jpg` 和 `assets/` 目录。加载器支持：
...
`components/agent-editor/agentEditConfig.ts` 将配置分成：
...
这里不是仅对当前会话生效的临时覆盖层。`ParametersSidebar` 的双向绑定 setter 调用 `agentStore.updateAgent`，而 `updateAgent` 会立即调用 `persistAgent`。因此从侧边栏改动的模型选择、参数和预设消息都会进入当前 Agent 的持久化配置；重新打开 Agent 或重启应用后仍会保留。侧边栏页签本身的激活状态属于 UI 状态，由 `useLlmChatUiState` 管理，不属于 Agent 配置。
...
`presetMessages` 是 `ChatMessageNode[]`，每个节点有 `id`、`parentId`、`childrenIds`、`role`、`content`、`status` 和 `isEnabled`。它可以表达：
...
配置向导明确要求实际配置包含 `chat_history` 锚点，否则没有位置放入真实对话；通常也应提供 `user_profile` 锚点。内置 YAML 角色普遍通过 system 角色设定 + user/assistant 示例 + chat history 形成角色卡式上下文。
...
- `parameters.contextCompression`：按 Token/消息规模触发非破坏性摘要压缩；
- `parameters.contextPostProcessing`：合并 system 消息、合并连续角色、把 system 转 user、强制 user/assistant 交替；
- `parameters.contextManagement`：最大上下文 Token 和截断时保留字符数；
- `parameters.imageCompression`：发送前图片格式、质量和最大尺寸；
