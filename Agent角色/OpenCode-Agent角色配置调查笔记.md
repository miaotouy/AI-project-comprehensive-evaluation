...
OpenCode 的 Agent 是「从配置构建的只读内存对象」，本身不落库；持久化的只是 session 表上的 agent 名字引用（`packages/core/src/session/sql.ts:51`）。Agent 配置来源为 `opencode.json/jsonc` 的 `agent` 字段、`{agent,agents}/**/*.md` 与 `{mode,modes}/*.md` 文件（mode 强制 primary）。最终 system prompt 由「agent.prompt（缺省用 provider 风格提示）→ env → AGENTS.md 指令 → MCP 指令 → skills」按序拼装（`src/session/llm/request.ts:56-66`、`src/session/prompt.ts:1257-1271`）。权限求值采用 agent 权限 + 会话权限合并的 allow/ask/deny 三档规则（`src/permission/index.ts`），工具的最终可见性在请求组装时按合并后的规则过滤。

关键事实（快照 1f94d8a）：

- **Agent 实体字段**：`name/description/mode("primary"|"subagent"|"all")/native/hidden/temperature/topP/color/permission/model/variant/prompt/options/steps`（`packages/opencode/src/agent/agent.ts:35-56`）。
- **加载与合并十一步**：远程 well-known → 全局 → `OPENCODE_CONFIG` → 项目 → `.opencode/` 目录 → `OPENCODE_CONFIG_CONTENT` → Console/Org → 企业托管 → mode 并入 agent → `OPENCODE_PERMISSION` → 全局 tools 并入 permission（`src/config/config.ts:314-596`）。
...
- `Agent.Info`（Effect Schema，agent.ts:35-56）：`name`、`description`、`mode`（`"subagent"|"primary"|"all"`）、`native`、`hidden`、`topP`、`temperature`、`color`、`permission`（`PermissionV1.Ruleset`）、`model`（`{modelID, providerID}`）、`variant`、`prompt`、`options`（任意 key-value，如温度等模型参数）、`steps`。
- 一次性在 `InstanceState` 闭包中构建（agent.ts:98-353），每次 `get/list/defaultInfo` 返回快照（:312-344）。**没有 agent 表**。
- 会话持久化的只有名字：session 表 `agent` 列（sql.ts:51），另有 `model` JSON 列（sql.ts:52-56）与 `permission` JSON 列（:50）。

### 1.2 V2 实体（迁移中）

- `packages/schema/src/agent.ts:20-37`：`id/model/request(headers+body)/system/description/mode/hidden/color/steps/permissions`；无独立 temperature/topP 字段（静态推断：走 `request.body`）。
- 内存 Map（core/src/agent.ts:48-66），同样不落库。

### 1.3 mode 语义（两套一致）

- `primary`：主 agent，可直接选择；`subagent`：只能被 task 工具调用；`all`：两者皆可。
- 可选择性：V1 `defaultInfo` 拒绝 `subagent`/`hidden`（agent.ts:328-340）；UI 主选择器过滤 `mode!=="subagent" && !hidden`（app/src/context/local.tsx:70），`@` 提及菜单只列非 primary 的 agent（app/src/components/prompt-input.tsx:582-605）。

## 2. 创建、选择与会话绑定

- **创建**：配置文件手写；`opencode agent create`（cli/cmd/agent.ts:33-232）由 LLM 生成（`Agent.generate`，agent.ts:368-436）后写成带 frontmatter 的 markdown（cli/cmd/agent.ts:195-222）。
- **选择器**：App 输入框 agent 下拉（prompt-input.tsx:1649-1675）；TUI `dialog-agent.tsx` + `agent.list/cycle` 命令（tui/src/app.tsx:678-736）；CLI `opencode run --agent`（cli/cmd/run.ts:170、596-639，拒绝 subagent 作主 agent）；HTTP `GET /app/agents`（server/routes/instance/httpapi/handlers/instance.ts:80-81）。
- **会话绑定**：`Session.createNext` 接受 agent 写入（session.ts:501-540）；每次用户消息若 agent/model 变化则 `setAgentModel` 更新 session 行（prompt.ts:672-689 → session.ts:767-778）。每条消息也带 agent 快照：user 消息 `info.agent`（prompt.ts:656-670）、assistant 消息 `agent/mode`（prompt.ts:1186-1200）。
- **切换**：V1 无专用端点，切换 = 下次 prompt 带不同 `input.agent` 覆盖 session 行；plan/build 互切由 `plan_enter`/`plan_exit` 权限 + plan 工具触发（src/tool/plan.ts:34-73）。V2 有 `session.switchAgent` 端点（server/src/handlers/session.ts:108-122 → core/src/session.ts:393-401）。
...
```
- provider 风格 prompt 按模型 api id 选择（system.ts:27-42，anthropic.txt/gpt.txt/gemini.txt/default.txt 等）。Meta 系模板覆盖全部 muse 家族：api id 含 `"muse"` 即返回 `PROMPT_META`，按 `muse-glimmer` 区分 Muse Glimmer/Muse Spark 并替换模板中 `{{MODEL_NAME}}` 占位（system.ts:27-31、prompt/meta.txt，b9f3b38）。
- 之后触发 `experimental.chat.system.transform` 插件钩子（request.ts:69-73）；OpenAI OAuth 时改走 `options.instructions`（request.ts:99，后经 llm.ts:316 的 `ProviderTransform.providerOptions` 注入；真正写 `providerOptions.instructions` 的是 Agent.generate 的 isOpenaiOauth 分支，agent.ts:418-433）。

**指令（AGENTS.md）加载**（src/session/instruction.ts）：
- `systemPaths`（:110-153）：全局 `~/.config/opencode/AGENTS.md`（及可选 `~/.claude/CLAUDE.md`，:60-63）→ 项目向上查找 `AGENTS.md`/`CLAUDE.md`/`CONTEXT.md`（CONTEXT.md 标注 deprecated，:64-68；每层只取第一个匹配，:122-133）→ `config.instructions` 指定路径/URL。
- `system` 渲染为 `Instructions from: {path}\n{content}`（:155-169）；就近指令文件只附加一次（claims 集合，:70-77、201-211）。

V2 运行时（core/src/session/runner/llm.ts:168-214）：`system = [agent.info?.system, system.baseline]`，baseline 来自 SystemContext 组合（instructions/skill-guidance/reference-guidance），按 registry key 排序输出（core/src/system-context/registry.ts:39-44）。

## 4. 模型、Provider 与生成参数

- **agent 可绑定**：`model`（字符串，`Provider.parseModel` 解析，agent.ts:281）、`variant`、`temperature`、`top_p`（agent.ts:282-286）、`options`。
- **生效点**：`prompt.ts:646`：`model = input.model ?? ag.model ?? currentModel(sessionID)`；`currentModel` 依次查 session 表 model 字段 → 最近带 model 的 user 消息 → `provider.defaultModel()`（prompt.ts:614-633）。
- **参数合并顺序**（request.ts:84-128）：`temperature = agent.temperature ?? ProviderTransform.temperature(model)`、`topP = agent.topP ?? ProviderTransform.topP(model)`、`options` 合并 `base(provider) → model → agent → variant`（:84-91）。
- **切换 agent 的模型行为**：App 端 `agent.set()` 保存 `{agent, model: item.model ?? prev.model, variant}`（app/src/context/local.tsx:196-216），即切换时默认继承上一个模型，除非新 agent 自带 model。V2 `switchAgent` 不改 session.model（core/src/session.ts:393-401）。
- **默认模型**：`Provider.defaultModel()`（provider.ts:1947-1980）：`cfg.model` → state/model.json 最近使用 → 第一个已配置 provider 的排序首个模型。App 端「当前默认模型」优先取 server `/config/providers` 响应中的 `defaultModel`（provider-catalog.ts:28-38），`cfg.model` 字符串仅作回退（941e71d）。
- **内置 title agent** 自带 `temperature: 0.5`（agent.ts:240）。

## 5. 工具、知识库、记忆与子 Agent

- **tools 字段（已废弃）**：`Schema.Record(Schema.String, Schema.Boolean)`（v1/config/agent.ts:21-23），normalize 时 `write/edit/patch` 映射为 `edit` 权限、其余键按工具名直接成为权限键（:69-77）。**本快照未找到 `"*"`、对象 map 或 "read-only" 组语法**（全局搜索无匹配）。全局 `config.tools` 同为布尔表并入全局 permission（config/config.ts:553-564）。
- **permissions 语法**：值可为字符串动作或 `{pattern: action}` 对象（v1/config/permission.ts:5-12；已知键 read/edit/glob/grep/list/bash/task/external_directory/todowrite/question/webfetch/websearch/lsp/doom_loop/skill，:17-36）。
- **运行时合流**：工具上下文 `ask` 用 `merge(agent.permission, session.permission)`（src/session/tools.ts:81-89）；`resolveTools` 合并后按 `Permission.disabled` 过滤（llm/request.ts:208-214）；MCP 工具可见性经 `Permission.visibleTools` 过滤（调用点在 src/tool/registry.ts:281，另 code-mode.ts:210；tools.ts:390-489 是 MCP 工具注册与 ask 求值）。会话级 permission 来源：`session.create` payload（session.ts:260-271）与 prompt payload 的 `tools`（废弃，prompt.ts:1060-1067）。
- **Skill 授权**：通过权限键 `skill` 控制：`Skill.available(agent)` 过滤 `Permission.evaluate("skill", name, agent.permission) !== "deny"`（src/skill/index.ts:310-315）；系统提示 skills 段与 skill 工具都走该过滤。
- **MCP 按 agent 过滤**：配置层无 per-agent 字段（core/src/v1/config/mcp.ts），全部在运行时按工具权限过滤（system.ts:112-117、tools.ts:390-489）。
- **TaskTool 子 agent 权限继承**：子会话权限 = 父会话 deny 规则 + `external_directory` 规则（agent/subagent-permissions.ts:14-27），子 agent 自身无 `todowrite`/`task` 权限则补 deny（:24-25）；task.ts:139-155 再追加 `task`/`todowrite` deny 与 `experimental.primary_tools` deny；`subagent_depth` 限制嵌套（task.ts:104-117）。
...
- **V1→V2 迁移**：`ConfigMigrateV1`（core/src/v1/config/migrate.ts:35-125）：`agent`/`mode` → `agents`（mode 强制 primary，:100）、`permission`/`tools` → `permissions`（write/patch→edit，:93-94）、`prompt` → `system`、`disable` → `disabled`、temperature/top_p 进 `request.body`（:106-125）。
- **frontmatter 解析**：markdown agent 文件的 YAML frontmatter 用 gray-matter 解析（core/src/config/markdown.ts:4-10）。

## 8. 配置 UI 与运行时可见性

- **App**：输入框 agent 下拉（prompt-input.tsx:1649-1675）与模型下拉（:1677-1709）；`@` 补全 subagent（:582-605）；侧栏按最后 user 消息的 agent 着色（app/src/pages/layout/sidebar-items.tsx:172、app/src/utils/agent.ts:34-44）。
- **TUI**：agent 选择对话框（tui/src/component/dialog-agent.tsx:11-26）；会话页按消息 agent 着色（tui/src/routes/session/index.tsx:1374、1543）；子 agent footer（tui/src/routes/session/subagent-footer.tsx:17-21）。
...
- 公共默认权限 `defaults`（agent.ts:119-136）：`*:allow`、`doom_loop:ask`、`external_directory:ask`（白名单目录 allow：Truncate.GLOB、tmp、skills、references）、`question/plan_enter/plan_exit:deny`、read 的 `.env` 特殊规则。
