...
AgentToolManager 的定义集合分为文件系统和命令工具（`read`、`write`、`edit`、`glob`、`grep`、`exec`、`process`，`:704-843`）、`question`、计划、Tape、memory、图片生成、cron、subagent、Skills 和设置/浏览器工具（`:399-570`、`:844-850`、`:1978-2048`）。每个定义带 `TOOL_EXECUTION` 合同：读取操作可标记 sequential/parallel，写入操作固定 sequential。
...
命令工具在 `CommandPermissionService`（`src/main/tool/permission/commandPermissionService.ts:26-53`、`:109-188`）另外做 base command、signature 和风险等级判断。`ls`、`pwd`、`echo`、`cat`、`head`、`tail`、`wc`、`grep`、`diff`、`find`、`sort`、`uniq` 属于安全白名单；网络、链式执行、破坏性模式会被判为 critical，审批可按会话缓存 signature（`:128-154`）。文件工具还通过 workspace/session 路径检查（`src/main/tool/agentTools/agentToolManager.ts:1494-1509`、`:1764-1833`）。
...
- MCP/Agent 工具合并与路由：`src/main/tool/index.ts:127-239`、`:311-430`
