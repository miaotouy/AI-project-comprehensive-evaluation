...
另有一个特殊工具 `kb_exec`（[`tools/knowledge_fs.py`](../../open-webui/backend/open_webui/tools/knowledge_fs.py) 1125-1183 行）：把知识库当“文件系统”的类 shell 命令工具，支持 `ls/tree/cat/head/tail/sed/grep/find/wc/stat` 及管道。匹配预算由 `match_budget`（55 行）限制，输出超过 `KB_EXEC_MAX_OUTPUT_CHARS` 会被截断；是否注入它由 `ENABLE_KB_EXEC` 决定（关闭时改用传统组合工具）。

## 2. 工具注册与加载

### 2.1 数据库工具（utils/tools.py `get_tools` 267-478 行）

- 数据库工具按以下顺序加载（utils/tools.py `get_tools` 267-478 行）：
  1. 批量读取 `Tools.get_tools_by_ids`（281 行）；
  2. `AccessGrants.has_access` 授权校验（287-299 行）；
  3. `load_tool_module_by_id` 模块缓存（301-307 行）；
  4. Valves 注入（314-320 行）；
  5. spec 清洗：`str`→`string` 类型修正、剥离 `__` 前缀保留参数；
  6. `get_async_tool_function_and_apply_extra_params` 绑定 `__id__`/`__user__`；
  7. 函数名冲突时加 `{tool_id}_` 前缀（365-369 行）。
...
- 代码解释器自动检测循环（5325-5395 行）：`open_webui:code_interpreter` 输出项触发，`DETECT_CODE_INTERPRETER` 最多重试 5 次。引擎分发——pyodide 经 `event_caller({'type': 'execute:python'})` 推给前端浏览器，jupyter 调 `execute_code_jupyter`；`CODE_INTERPRETER_BLOCKED_MODULES` 注入受限 `__import__` 包装代码（5350-5368 行）。
- Responses API 有状态模式（5230-5237 行）与普通模式（5239-5277 行）的拼接路径不同。

## 4. MCP 与外部工具服务器

- `connect_mcp_server`（middleware.py 2197-2245 行）的连接步骤：
  1. 查 `tool_server.connections` 中的 mcp 连接，做 `has_connection_access` 校验；
  2. `build_tool_server_headers` 组装鉴权头（bearer/session/oauth）；
  3. `MCPClient.connect` 建连（utils/mcp/client.py，`streamablehttp_client` + `OAuthClientProvider`）；
  4. `list_tool_specs` 拉取工具清单；
  5. `function_name_filter_list` 过滤（2237-2243 行）。
...
- `utils/plugin.py`：提供工具模块缓存、加载、import 替换与 Valves 动态 options 解析等函数（`get_tools_cache`/`get_tool_module_from_cache`/`load_tool_module_by_id`/`replace_imports`/`resolve_valves_schema_options`，27 行）；插件加载行为受 `ENABLE_PLUGINS`、`OFFLINE_MODE`、`ENABLE_PIP_INSTALL_FRONTMATTER_REQUIREMENTS`、`PIP_OPTIONS` 四个开关控制；
