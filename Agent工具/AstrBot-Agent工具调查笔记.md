...
- **四路注册汇合**：内置工具（`@builtin_tool` 装饰器，registry.py:232-254）、插件工具（star_handler.py:670-724 的 `add_func`/`spec_to_func`）、MCP 工具（`mcp_server.json` → `init_mcp_clients`，func_tool_manager.py:542-644）、子 Agent（`HandoffTool`，handoff.py:8-64）。
- **同名工具去重规则为「active 优先、同状态后覆盖」**：`ToolSet.add_tool`（tool.py:91-108），`get_func` 反向扫描配合（func_tool_manager.py:399-417）。
- **执行串行无并行**：`_handle_function_tools`（tool_loop_agent_runner.py:1031-1297）对同一响应中的多个 tool_call 用 `zip` 顺序 for + 逐个 await（:1050-1054）。
- **工具返回 None = 结束 Agent**：`_execute_local` 中工具直接给用户发消息后 yield None（astr_agent_tool_exec.py:680-697），runner 收到 None 转 `AgentState.DONE`（tool_loop_agent_runner.py:1225-1240）。
- **安全分层**：非内置工具经 `_PermissionGuardedTool` 透明代理按 `tool_permissions`（SharedPreferences `_default` 键）查权限（func_tool_manager.py:214-285、460-496），默认 member 不限制；内置敏感工具在自身实现内硬编码检查。
- **五种产物级防护**：工具超时（`asyncio.wait_for`）、结果 token 溢出落盘（>27.5k tokens 写文件，tool_loop_agent_runner.py:110、397-460）、重复调用提示守卫（同工具同参连续 ≥3 次注入系统提示，:145-174、668-693）、`MAX_STEPS` 截断（:1002-1029）、`skills_like` 双段 requery（:289-307、1337-1401）。
...
- **MCP 生命周期自管理**：启动并行初始化（并发 task + `asyncio.gather`）、连接超时/启用超时双环境变量、幂等启动、同任务内清理（#9068 修复）、ModelScope 云端同步。
...
用户消息 → astr_main_agent.run_agent (astr_agent_run_util.py:115)
  ...
  → step() 循环：
      ...
      → _handle_function_tools（tool_loop_agent_runner.py:1031-1297）
          for 每个 tool_call（串行）：
            查工具（skills_like 模式查 raw set，否则查 req.func_tool）
            参数过滤（仅传 parameters.properties 声明过的键，:1104-1130）
...
| MCP 工具 | `data/mcp_server.json` → `init_mcp_clients`（:542-644） | `func_list`（`MCPTool` 包装，:706-712） | 启动时并行初始化；重连时先清除同名 MCPTool（:701-705） |
...
### 2.3 工具启停持久化（func_tool_manager.py:988-1045）

- `deactivate_llm_tool`（:988-1015）：置 `active=False` + 写入 SharedPreferences `inactivated_llm_tools`；
- `activate_llm_tool`（:1018-1045）：若工具属插件且插件未激活则抛 `ValueError`（:1021-1025），置 active 并移除黑名单；
- 插件侧 API：`star/context.py:372-383`（`activate_llm_tool`/`deactivate_llm_tool`），`register_llm_tool`（:718）。

## 3. 运行时工具集构建

### 3.1 get_full_tool_set（func_tool_manager.py:498-515）
...
- 执行时工具解析走 raw set（:1080-1086）；第二轮以 param-only schema requery 参数（`_resolve_tool_exec`，:1337-1401，含失败后更强指令重试 `SKILLS_LIKE_REQUERY_REPAIR_INSTRUCTION` :136-144）。
...
### 4.3 runner 侧消费（tool_loop_agent_runner.py:1031-1297）

- 逐个工具：先 yield `tool_call` 消息链（:1060-1074，Json 含 id/name/args/ts），供 UI 展示；
- 未找到工具：yield `error: Tool X not found. Available tools are: ...`（:1096-1102）；
- **参数过滤**（:1104-1130）：有 handler 时只传 `parameters.properties` 声明过的键，多余键记 warning 忽略；无 handler（MCP 等）用全部参数；
- `on_tool_start` / `on_tool_end` hooks（:1132-1139、:1254-1262）；
- 结果处理（:1147-1252）：
  - `TextContent` → 文本拼接；
  - `ImageContent` → `tool_image_cache.save_image` 缓存（base64 落盘），给 LLM 的文本提示"Review the image below. Use send_message_to_user..."（:1163-1180）；
  - `EmbeddedResource`（Text/Blob）同规则，非图片资源回"不支持的数据类型"（:1181-1210）；
  - 大结果 `_materialize_large_tool_result`（:1213-1216）落盘；
  - `resp is None` → 转 `AgentState.DONE`（:1225-1240）；
- 异常兜底：任何执行异常以 `error: {e}` 作为 tool 结果回填（:1263-1273），`_ToolExecutionInterrupted` 特殊重抛；
- `_iter_tool_executor_results`（:1467-1509）：`asyncio.wait` 竞争 next_result 与 abort 信号，用户停止可中断工具执行。

## 5. 工具结果处理与上下文回填

### 5.1 大结果落盘（:110-111、:370-460）

- `TOOL_RESULT_MAX_ESTIMATED_TOKENS = 27_500`、`TOOL_RESULT_PREVIEW_MAX_ESTIMATED_TOKENS = 7000`（:110-111）；
- `_materialize_large_tool_result`（:397-460）：估算 token 超限 → 预览截断到 7000 + 全文写入 `tool_result_overflow_dir`（文件名 = 净化后的 tool_call_id + uuid 后 8 位，:380-388），回填文本附 `TOOL_RESULT_OVERFLOW_NOTICE_TEMPLATE` 提示用读文件工具查看（:169-174）；
- 写盘用 `asyncio.to_thread`（:395）。

### 5.2 重复调用守卫（:145-174、:668-693）

- `_track_tool_call_streak`（:668）：同工具名 + 同参数连续调用计数；
- 阈值 3/4/5 三档，L1 温和提醒、L2 强调、L3 严正提醒，模板追加到 tool 结果尾部（:149-168）；
- 参数序列化比较：连续两次完全相同的工具+参数才累加。

### 5.3 回填与 UI 消息链

- 工具调用/结果以 `MessageChain(type="tool_call")` / `type="tool_call_result"`（Json 组件）yield 出去（:1060-1074、:1275-1290）；
...
### 6.1 权限模型（func_tool_manager.py:214-285、453-496）
...
- 权限读取：SharedPreferences `tool_permissions`（global scope）→ `_default` 键 → 工具名（:471-480）；默认 `_default_permission` = `"member"`（:453-458）即不限制；
...
| 结果大小 | 27.5k tokens 落盘 + 7k 预览 | tool_loop_agent_runner.py:110-111、397-460 |
| 重复调用 | 3/4/5 档提示注入 | :145-174、668-693 |
| 步数上限 | `MAX_STEPS`（max_agent_step 默认 30）+ `MAX_STEPS_REACHED_PROMPT` 拔工具强收尾 | :126-130、1002-1029 |
| 用户中断 | `request_stop()` → abort 信号中断执行器读取 | :1407-1411、1467-1509 |
| 参数白名单 | 仅传声明过的参数键 | tool_loop_agent_runner.py:1104-1130 |
| 图像引用净化 | `is_supported_image_ref` 过滤非法 image_urls | astr_agent_tool_exec.py:101-128 |
| 插件权限链路 | 插件未激活时 `activate_llm_tool` 抛错 | func_tool_manager.py:1021-1025 |
...
### 8.1 配置与初始化（func_tool_manager.py:542-644）

- 配置文件 `data/mcp_server.json`，格式 `{"mcpServers": {name: {command/args 或 url, active}}}`；缺失时自动写默认空配置（:569-575）；
- `init_mcp_clients`：过滤 `active=False`（:585），每个服务一个 task（name=`mcp-init:{name}`）`asyncio.gather` 并行（:594-606），失败记录并弹运行时表（:621-623）；
- 双超时：`ASTRBOT_MCP_INIT_TIMEOUT`（初始化）与 `ASTRBOT_MCP_ENABLE_TIMEOUT`（动态启用），默认值 `_resolve_timeout` 解析（:303-316）；
- 全部失败时按 `raise_on_all_failed` 抛 `MCPAllServicesFailedError`（:639-643）；
- 日志脱敏：`_log_safe_mcp_debug_config` 只记录可执行文件名/主机名不记录 command/url 全文（:517-540）。

### 8.2 生命周期（:646-764+）

- `_start_mcp_server` 幂等：已运行/启动中则忽略（:659-666）；
- 连接 + `list_tools_and_save` 在同一任务内完成，工具注册为 `MCPTool` 进 `func_list`（:700-712），重连前先移除旧同名 MCPTool（:701-705）；
- **清理必须在同一任务**：`shutdown_event.wait()` 后 `_terminate_mcp_client`（:721-742），注释引用 #9068——`asyncio.shield` 跨任务退出 anyio cancel scope 会失败；取消吸收循环兼容 Py3.10 无 `uncancel`（:735-742）。
...
- `sync_modelscope_mcp_servers`（func_tool_manager.py:1076-1143）：ModelScope OpenAPI 拉取操作中服务并合并本地配置。
...
- `inactivated_llm_tools` 黑名单与 `active` 标志双写（func_tool_manager.py:999-1012），保证跨重启持久；
- MCP 重连先清同名工具（:701-705），保证热更新不累积重复声明；
...
- 注册与生命周期：`astrbot/core/tools/registry.py`（builtin_tool :232-254、config 规则 :26-198、状态计算 :288-332）、`astrbot/core/provider/func_tool_manager.py`（_PermissionGuardedTool :214-285、add/remove/get :365-417、get_full_tool_set :498-515、权限 :453-496、MCP :542-764+、启停 :988-1045、配置读写 :1052-1074、ModelScope :1076-1143）
...
- 主循环：`astrbot/core/agent/runners/tool_loop_agent_runner.py`（reset/skills_like :207-327、大结果 :370-460、_handle_function_tools :1031-1297、requery :1299-1401、中断读取 :1467-1509）
