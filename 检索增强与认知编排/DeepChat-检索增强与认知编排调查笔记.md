...
DeepChat 在本类目的已确认主链是 Session Tape 的工具化检索。Tape 记录消息、事件、锚点、工具调用与工具结果等会话轨迹；模型可先用 `tape_search` 取得带来源和分数的紧凑结果，再用 `tape_context` 在选定条目周围展开受字节预算约束的证据。它是对可恢复研究轨迹的检索，不是面向独立文档库的摄取管线。
...
可与 Jan、Chatbox、Open WebUI 的工具化检索比较“模型何时搜索、search/read 粒度、结果来源和预算”。它也可与研究轨迹系统比较记录可追溯性和子 Agent 作用域。
