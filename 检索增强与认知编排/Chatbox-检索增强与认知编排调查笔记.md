...
知识库的作用域来自会话选择的单个知识库 ID，所有文件元数据、读取块和检索 IPC 都将该 ID 作为 SQL/索引边界；文件元数据查询最多 100 个文件 ID，读取最多 200 个块，分页单次最多 100 项（**源码事实**）。知识库工具未见逐文件 ACL、用户/租户标识或对文档中不可信指令的净化层（对 `knowledge-base` IPC、toolset 和查询实现的**静态检查结果**）。`src/main/knowledge-base/ipc-handlers.ts:324-442`。
